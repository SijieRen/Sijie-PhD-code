...
if 'mnist' in args.dataset:
    traintest_loader = DataLoader(get_dataset_2D(root=args.root, args=args, fold='traintest',
    ...
    traintest_loader = DataLoader(get_dataset_2D(root=args.root, args=args, fold='train',
    ...
    # if args.dataset == 'AD':
    traintest_loader = DataLoader(get_dataset(root=args.root, args=args, fold='train',
...
# Training Phase
history_loss = gmvae.train(traintest_loader, val_loader)
...
root = "./"
if 'mnist' in args.dataset:
    train_result = gmvae.test(train_loader)
    save_results_as_xlsx(root, "train_MI_E-Y_" +
                         args.xlsx_name, train_result, args)
    test_result = gmvae.test(test_loader)
    save_results_as_xlsx(root, "test_MI_E-Y_" +
                         args.xlsx_name, test_result, args)

test_result = gmvae.test(test_loader)
save_results_as_xlsx(root, "MI_E-Y_"+args.xlsx_name, test_result, args)
