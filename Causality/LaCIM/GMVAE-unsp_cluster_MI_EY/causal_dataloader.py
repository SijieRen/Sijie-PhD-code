...
class get_dataset(data.Dataset):
    def __init__(self,
        ...
        # unsp cluster train/test All In
        fold_map = {'train': "012", 'val': 1, 'test': "012"}
        ...
        if args.dataset == 'AD':
            ...
            if 'gene_5' in args.dataset_type:
                ...
                for rows in range(1, sheet.nrows):
                    # if str(int(sheet.row_values(rows)[6])) in fold_map[fold]: #unsp cluster train/test All In
                    self.fold.append(sheet.row_values(rows)[6])
                    self.image_path_list.append(os.path.join(
                        self.root, sheet.row_values(rows)[7]))
                    self.y.append(Label_map[sheet.row_values(rows)[2]])
                    self.u.append(sheet.row_values(rows)[8:12])
    ...
    def __getitem__(self, index):
        ...
        # torch.from_numpy(np.array(self.fold[index]).astype('int')), # for recording fold
...
class get_dataset_2D(data.Dataset):
    def __init__(self,
        ...
        self.transform = transform
        fold_map = {'train': "012", 'val': 1, 'test': "012"}
        fold_map_cmnist = {'train': 0, 'test': 1}
        self.fold = []
        self.fold_judge = fold
        if args.dataset == 'NICO':
            ...
            if args.dataset_type == 'NICO_0':
                ...
                sheet = workbook.sheet_by_index(10)
            for rows in range(1, sheet.nrows):
                # if str(int(sheet.row_values(rows)[4])) in fold_map[fold]:
                self.fold.append(sheet.row_values(rows)[4])
                self.image_path_list.append(os.path.join(
            ...
            if self.fold_judge == "traintest":
                fold_list = ["train", "test"]
                for fold in fold_list:
                    if self.root is None:
                        root__ = '../data/colored_MNIST_0.02_env_2_0_c_2/%s/' % fold
                    else:
                        root__ = self.root + '%s/' % fold

                    all_classes = os.listdir(root__)
                    for one_class in all_classes:  # one class -> 0123456789
                        for filename in os.listdir(os.path.join(root__, one_class)):
                            self.fold.append(fold_map_cmnist[fold])
...
def save_results_as_xlsx(root: str, xlsx_name: str, res_dict: Dict, args):
    ...
    if not os.path.exists(os.path.join(root, xlsx_name)):
        ...
        for ii in range(len(res_dict['predict_label'])):
            ...
            # print(res_dict['true_label'][ii])
            worksheet.write(row_idx, 1, res_dict['true_label'][ii][0])
            worksheet.write(row_idx, col_idx, res_dict['predict_label'][ii])
            worksheet.write(row_idx, 10, res_dict['fold'][ii])
