...
def train(epoch, model, optimizer, dataloader, args):
    ...
    for batch_idx, (x, target, env, u) in enumerate(dataloader):
        ...
        all_mu = [[] for i in range(args.env_num)]
        for ss in range(args.env_num):
            if torch.sum(env == ss) <= 1:
                continue
            part_target = target[env == ss]
            ...
            for cls_i in range(args.num_classes):
                all_mu[ss].append(s[part_target==cls_i, :])
        ...
        is_valid = 1
        mu_loss = torch.FloatTensor([0.0]).cuda()
        for ss in range(args.env_num):
            for cls_i in range(args.num_classes):
                if len(all_mu[ss][cls_i]) == 0:
                    #print('all_mu[ss][cls_i]', all_mu[ss][cls_i])
                    is_valid = 0
        if is_valid:
            for cls_i in range(args.num_classes):
                for s_i in range(args.env_num):
                    # print('norm for s_i %d, cls_i: %d'%(s_i, cls_i), torch.norm(all_mu[s_i][cls_i].mean(0)))
                    for s_j in range(s_i+1, args.env_num):
                        mu_loss = torch.add(mu_loss, F.mse_loss(all_mu[s_i][cls_i].mean(0), all_mu[s_j][cls_i].mean(0)))
        ...
        loss = torch.add(loss, recon_loss + args.beta * kld_loss + args.gamma * cls_loss + weight * mu_loss)
        if batch_idx % 10 == 0:
            print('raw loss w/o mu_loss:', (recon_loss.item() + args.beta * kld_loss.item() + args.gamma * cls_loss.item()))
        if weight > 1:
            loss = loss / weight
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        all_loss.update(loss.item() + mu_loss.item(), x.size(0))
        
        
        if batch_idx % 10 == 0:
            args.logger.info(
                'epoch [{}/{}], batch: {}, rec_loss:{:.4f}, kld_loss:{:.4f} cls_loss:{:.4f}, mu_loss:{:e},l2_weight:{:e}, overall_loss:{:e},acc:{:.4f}'
                .format(epoch,
                        args.epochs,
                        batch_idx,
                        RECON_loss.avg,
