...
def evaluate_22_randn(epoch, model, dataloader, args):
    model.eval()
    model.zero_grad()
        ...
        with torch.no_grad():
            for ss in range(args.sample_num):
                for env_idx in range(args.env_num):
                    _, _, mu, logvar, z_q, s_q, _ = model(x, env_idx, feature=1)
                    z, s = torch.randn(x.size(0), args.zs_dim // 2).cuda(), torch.randn(x.size(0),
                                                                                        args.zs_dim // 2).cuda()
                    
                    # print(mu, logvar.mul(0.5).exp_())
                    #print(z, z_q)
                    # print(F.mse_loss(z, z_q), F.mse_loss(s, s_q))
                    # print(z.mean(), z_q.mean())
                    # print(z.std(), z_q.std())
                    recon_x, _ = model.get_x_y(z, s)
                    if z_init is None:
                        z_init, s_init = z, s
...
def main():
    ...
    for optim_type in ['Adam']:# , 'sgd'
        for test_ep in [10]:
            for lr2 in [0.001]:
                for wd2 in [0.0001]:
                    for sample_num in [1, 5, 10,50,100,200,500, 1000]:
                        temp_args = copy.deepcopy(args)
                        ...
                        temp_args.lr2 = lr2
                        temp_args.reg2 = wd2
                        pred_test, test_acc = evaluate_22_randn(1, model, test_loader, temp_args)
