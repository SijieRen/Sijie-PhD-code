...
def train(epoch, model, optimizer, optimizer_y, dataloader, args):
    ...
    for batch_idx, (x, target, env, u) in enumerate(dataloader):
        ...
        irm_loss = args.alpha * irm_loss / x.size(0)
        ...
        optimizer_y.step()
        all_loss.update(loss.item()+irm_loss.item(), x.size(0))
...
def main():
    ...
    for name, param in model.named_parameters():
        # print(name)
        if 'Dec_y' not in name:
            params_list.append(param)
    if args.optimizer == 'sgd':
        optimizer = optim.SGD(params_list, lr=args.lr, momentum=args.momentum, weight_decay=args.reg)
        optimizer_y = optim.SGD(model.Dec_y.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.reg)
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        adjust_learning_rate(optimizer_y, epoch, args.lr_decay, args.lr_controler)
        _, _ = train(epoch, model, optimizer, optimizer_y, train_loader, args)
