    ...
    for batch_idx, (x, target, env, u) in enumerate(dataloader):
        ...
        for ss in range(args.env_num):
            ...
            for cls_i in range(args.num_classes):
                if ss == 0:
                    all_mu_0.append(s[part_target==cls_i, :])
                else:
                    all_mu_1.append(s[part_target==cls_i, :])
        ...
        if len(all_mu_0[0]) > 0 and  len(all_mu_0[1]) > 0 and len(all_mu_1[0]) > 0 and len(all_mu_1[1]) > 0:
            mu_loss = args.alpha * F.mse_loss(all_mu_0[0].mean(0), all_mu_1[0].mean(0)) + \
                      args.alpha2 * F.mse_loss(all_mu_0[1].mean(0), all_mu_1[1].mean(0))
        ...
        loss = torch.add(loss, recon_loss + args.beta * kld_loss + args.gamma * cls_loss + mu_loss)
