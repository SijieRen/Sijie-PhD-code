...
def VAE_loss(recon_x, x, mu, logvar, args):
    ...
    """
    BCE = F.binary_cross_entropy(recon_x.view(-1, 1 * args.image_size ** 3), x.view(-1, 1 * args.image_size ** 3),
                                 reduction='mean')
    ...
    for batch_idx, (x, u, us, target, env) in enumerate(dataloader):
        for ss in range(args.env_num):
            if ss == 0:
                ...
                _, recon_x, mu, logvar, z, s, zs = model(x, ss, feature=1, is_train = 1)
                pred_y = model.get_y_by_zs(mu, logvar, ss)
                recon_loss_t, kld_loss_t = VAE_loss(recon_x, x, mu, logvar, args)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        _, _ = train(epoch, model, optimizer, train_loader, train_loader_2, args)
