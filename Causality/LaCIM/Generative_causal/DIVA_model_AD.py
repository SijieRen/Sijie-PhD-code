...
class px(nn.Module):
    def __init__(self, d_dim, x_dim, y_dim, zd_dim, zx_dim, zy_dim):
        ...
        self.de1 = nn.Sequential(nn.ConvTranspose3d(16, 64, kernel_size=5, stride=1, padding=0, bias=False), nn.BatchNorm3d(64), nn.ReLU(),
                                 nn.Conv3d(64, 128, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm3d(128), nn.ReLU(),)
        self.up2 = nn.Upsample(24)
        self.de2 = nn.Sequential(nn.ConvTranspose3d(128, 128, kernel_size=5, stride=1, padding=0, bias=False), nn.BatchNorm3d(128), nn.ReLU(),
                                 nn.Conv3d(128, 128, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm3d(128), nn.ReLU(),)
        self.up3 = nn.Upsample(48)
        self.de3 = nn.Sequential(nn.Conv3d(128, 32, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm3d(32), nn.ReLU(),
...
class qzd(nn.Module):
    def __init__(self, d_dim, x_dim, y_dim, zd_dim, zx_dim, zy_dim):
        ...
        self.encoder = nn.Sequential(
            nn.Conv3d(1, 64, kernel_size=3, stride=2, padding=1, bias=False), nn.BatchNorm3d(64), nn.ReLU(),
            nn.Conv3d(64, 128, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm3d(128), nn.ReLU(),
            nn.MaxPool3d(3),
            nn.Conv3d(128, 256, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm3d(256), nn.ReLU(),
            nn.Conv3d(256, 256, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm3d(256), nn.ReLU(),
            nn.MaxPool3d(2),
            nn.Conv3d(256, 256, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm3d(256), nn.ReLU(),
            nn.Conv3d(256, 128, kernel_size=3, stride=1, padding=1, bias=False), nn.BatchNorm3d(128), nn.ReLU(),
            nn.MaxPool3d(2),
        )
...
class DIVA(nn.Module):
    ...
    def loss_function(self, d, x, y=None):
        if y is None:  # unsupervised
            ...
            CE_x = F.mse_loss(x_recon, x, reduction='sum')
            
            # x_recon = x_recon.view(-1, 256)
            # x_target = ((x.view(-1)*0.5 + 0.5) * 255).long()
            #
            ...
            # print(x_recon.size(), x.size())
            CE_x = F.mse_loss(x_recon, x, reduction='sum')
            # x_recon = x_recon.view(-1, 256)
            # x_target = ((x*0.5+0.5).view(-1) * 255).long()
            # print(x_recon.size(), x_recon)
            # print(x_target.size(), x_target)
            # CE_x = F.cross_entropy(x_recon, x_target, reduction='sum')
            ...
            # print('CE_x', CE_x, 'self.aux_loss_multiplier_d', self.aux_loss_multiplier_d,
            #       'self.aux_loss_multiplier_d * CE_d', self.aux_loss_multiplier_d * CE_d,
            #       'self.aux_loss_multiplier_y', self.aux_loss_multiplier_y,
            #       'self.aux_loss_multiplier_y * CE_y', self.aux_loss_multiplier_y * CE_y)
