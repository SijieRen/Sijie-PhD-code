...
def train(epoch, model, optimizer, dataloader, args):
    ...
    for batch_idx, (x, target, env, u) in enumerate(dataloader):
        ...
        #rex_loss = torch.var(torch.cat(cls_loss_all, dim=0))
        #print(cls_loss_all, rex_loss)
        rex_loss = torch.FloatTensor([0.0]).cuda()
        for s_i in range(args.env_num):
            for s_j in range(s_i + 1, args.env_num):
                rex_loss = torch.add(rex_loss, (cls_loss_all[s_i] - cls_loss_all[s_j]) ** 2)
        ...
        loss = torch.add(loss, recon_loss + args.beta * kld_loss + args.gamma * cls_loss + weight * rex_loss)
        if weight > 1:
            loss = loss / weight
        ...
        if batch_idx % 10 == 0:
            args.logger.info(
                'epoch [{}/{}], batch: {}, rec_loss:{:.4f}, kld_loss:{:.4f} cls_loss:{:.4f}, rex_loss:{:e} overall_loss:{:e},acc:{:.4f}'
                .format(epoch,
                        args.epochs,
                        batch_idx,
                        RECON_loss.avg,
...
def main():
    ...
    if args.model == 'VAE' or args.model == 'VAE_old':
        ...
        if args.dataset == 'AD':
            ...
            elif args.smaller_net == 2:
                model = Generative_model_f_2D_unpooled_env_t_mnist_more_indvidual_small(in_channel=args.in_channel,
                                                             u_dim=args.u_dim,
                                                             us_dim=args.us_dim,
                                                             zs_dim=args.zs_dim,
                                                             num_classes=args.num_classes,
