...
def main():
    ...
    if args.model == 'VAE' or args.model == 'VAE_old':
        ...
        model = Generative_model_f_2D_unpooled_env_t_mnist_more_share(in_channel=args.in_channel,
                                                           u_dim=args.u_dim,
                                                           us_dim=args.us_dim,
                                                           zs_dim=args.zs_dim,
                                                           num_classes=args.num_classes,
    ...
    if args.inference_only and args.load_path != '':
        ...
        test_loader_NICO = DataLoaderX(get_dataset_NICO_inter(args=args,
                                       ...
                                       num_workers=1,
                                       pin_memory=True)
        pred_test, label_test, test_acc = evaluate_22_only(model, test_loader_NICO, args)
    ...
    xlsx_name = '%s_model_unpooled_%s_u_%d_fold_%d.xls' % \
                (os.path.basename(sys.argv[0][:-3]), args.model, args.is_use_u, args.fold)
