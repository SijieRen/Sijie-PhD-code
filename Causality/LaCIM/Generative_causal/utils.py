    ...
    def __init__(self,
        ...
        if args.dataset == 'AD':
            ...
            if args.dataset_type == 'gene_1':
                ...
                if args.fold == 1:
                    workbook = xlrd.open_workbook(r"%sDataset/ADNI_dataset_APOE4.xlsx"%self.root)
                    print('load data excel 1')
                elif args.fold == 2:
                    workbook = xlrd.open_workbook(r"%sDataset/ADNI_dataset_APOE4_2.xlsx"%self.root)
                    print('load data excel 2')
                elif args.fold == 3:
                    workbook = xlrd.open_workbook(r"%sDataset/ADNI_dataset_APOE4_3.xlsx"%self.root)
                    print('load data excel 3')
                if 'SEX' in args.dataset_type:
                    sheet = workbook.sheet_by_index(10)
...
class get_dataset_2D(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_1, \
               ...
               torch.from_numpy(np.array(self.u[index]).astype('float32').reshape((len(self.u[index]))))
    ...
    def __init__(self,
        ...
        for files in os.listdir(os.path.join(self.root, './NICO/replace/')):
            if files[-4:] == '.png' or files[-4:] == '.jpg':
                self.image_path_list.append(os.path.join(self.root, './NICO/replace/', files))
                if 'cat' in files:
                    self.y.append(0)
                else:
                    self.y.append(1)
...
def get_opt():
    ...
    parser.add_argument('--hidden', type=int, nargs='+', default=[512, 1024, 512], help='')
...
def VAE_loss(recon_x, x, mu, logvar, mu_prior, logvar_prior, zs, args):
    ...
    eps = 1e-5
    if args.dataset == 'AD':
        BCE = F.binary_cross_entropy(recon_x.view(-1, 48 ** 3), x.view(-1, 48 ** 3), reduction='mean')
    elif 'mnist' in args.dataset:
        x = x * 0.5 + 0.5
        BCE = F.binary_cross_entropy(recon_x.view(-1, 3 * 28 ** 2), x.view(-1, 3 * 28 ** 2), reduction='mean')
    else:
        x = x * 0.5 + 0.5
        BCE = F.binary_cross_entropy(recon_x.view(-1, 3 * 256 ** 2), x.view(-1, 3 * 256 ** 2), reduction='mean')
...
def VAE_old_loss(pred_y, recon_x, x, mu, logvar, mu_prior, logvar_prior, zs, q_y_s, y, args):
    ...
    """
    eps = 1e-5
    BCE_all = F.binary_cross_entropy(recon_x.view(-1, 48 ** 3), x.view(-1, 48 ** 3), reduction='none').mean(dim=1)
    if args.fix_mu == 1:
        mu_prior = torch.zeros(mu_prior.size()).cuda()
    if args.fix_var == 1:
        logvar_prior = torch.ones(logvar_prior.size()).cuda()
    ...
    for ss in range(x.size(0)):
        if ss == 0:
            ...
            BCE = torch.add(BCE, ((q_y_s[ss][y[ss]] / pred_y[ss][y[ss]])* BCE_all[ss]))
            KLD = torch.add(KLD, (q_y_s[ss][y[ss]] / pred_y[ss][y[ss]]) * (log_q_zs[ss] - log_p_zs[ss]).mean())
