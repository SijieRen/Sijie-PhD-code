...
def train(epoch, model, optimizer, dataloader, args):
    ...
    for batch_idx, (x, u, us, target, env) in enumerate(dataloader):
        ...
        gradients_env = []
        logits_env = []
        net_parameter, net_dummies = find_parameters(model)
        logits = model(x)
        logits_env.append(logits)
        losses_env.append(F.cross_entropy(model(x), target))
        gradients_env.append(
            grad(losses_env[-1], net_dummies, create_graph=True))

        # pred_y = model(x)
        # loss = torch.FloatTensor([0.0]).cuda()
        # irm_loss = torch.FloatTensor([0.0]).cuda()
        # ib_irm_loss = torch.FloatTensor([0.0]).cuda()
        # l2_reg = torch.tensor(0.).cuda()
        # for param in model.parameters():
        #     l2_reg += torch.norm(param)

        # loss = torch.add(loss, args.reg * l2_reg)
        # # TODO xiugai wei IBIRM loss
        ...
        accuracy.update(compute_acc(logits.detach().cpu().numpy(),
        ...
        obj = (1 - args.irm_lambda) * losses_avg
        obj += args.irm_lambda * penalty

        # if self.hparams['ib_on'] or (not self.args["ib_bool"]):
        obj += args.ib_lambda * logit_penalty

        optimizer.zero_grad()
        obj.backward()
        optimizer.step()
        all_loss.update(obj.item(), x.size(0))
        if batch_idx % 10 == 0:
            print(
                'epoch [{}/{}], batch: {}, overall_loss:{:.4f},acc:{:.4f}'
                .format(epoch,
                        args.epochs,
                        batch_idx,
                        all_loss.avg,
