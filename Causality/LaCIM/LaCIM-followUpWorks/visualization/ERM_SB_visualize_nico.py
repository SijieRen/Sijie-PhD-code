...
import xlrd
from func_for_nico_erm import compute_saliency_maps, make_fooling_image
...
def evaluate(epoch, model, dataloader, args):
    ...
    for batch_idx, (x, target, env, u) in enumerate(dataloader):
        # if batch_begin <= 100:
            ...
            img_list.append((30*X_fooling_difference).squeeze(0).detach().permute(
                1, 2, 0).cpu().numpy()*0.5 + 0.5)
            ...
            plt.axis('off')
            if not os.path.exists('./ERM_2_selection_bias_saved_img_grdient_NICO_train/'):
                os.makedirs('./ERM_2_selection_bias_saved_img_grdient_NICO_train/')
            plt.savefig('./ERM_2_selection_bias_saved_img_grdient_NICO_train/%03d.png' %
                        (batch_begin))
...
test_loader = DataLoaderX(get_dataset_2D_env_selection_bias(root=args.root, args=args, fold='train',
