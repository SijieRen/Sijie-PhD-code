...
def evaluate(epoch, model, dataloader, args):
    ...
    for batch_idx, (x, target, env, u) in enumerate(dataloader):
        # if batch_begin <= 100:
            ...
            img_list.append((10*X_fooling_difference).squeeze(0).detach().permute(
                1, 2, 0).cpu().numpy()*0.5 + 0.5)
            ...
            plt.axis('off')
            if not os.path.exists('./93_2_selection_bias_saved_img_grdient_NICO_train/'):
                os.makedirs('./93_2_selection_bias_saved_img_grdient_NICO_train/')
            plt.savefig('./93_2_selection_bias_saved_img_grdient_NICO_train/%03d.png' %
                        (batch_begin))
...
class params(object):
    def __init__(self):
        ...
        # 5.20网上继续试
        self.eval_path = '/home/ruxin2/PPA-classification-prediction/Causality/results//NICO/save_model_VAE_f_share_NICO_10_2_10.0_0.1_lr_0.0100_60_0.5000_wd_0.0005_2023-07-05_17-17-54'
