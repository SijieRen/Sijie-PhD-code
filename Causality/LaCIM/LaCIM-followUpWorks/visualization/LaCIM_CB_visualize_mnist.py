...
from models import Generative_model_f_2D_unpooled_env_t_mnist_prior_share
import xlrd
...
class params(object):
    def __init__(self):
        ...
        # TPAMI 2023
        # 9746 /results/mnist_2/save_model_VAE_f_share_test_32_4704.0_0.1_lr_0.0100_80_0.5000_wd_0.0005_2023-05-20_13-25-44
        # 2023-06-28 18:24:31,827 - AL - INFO - init_acc: 0.9849, after acc: 0.9698 = 2
        # 2023-06-28 18:24:31,871 - AL - INFO - model save path: ../results/mnist_2/save_model_VAE_f_share_test_32_4704.0_0.1_lr_0.0100_80_0.5000_wd_0.0005_2023-06-28_15-30-39/

        self.eval_path = '/home/ruxin2/PPA-classification-prediction/Causality/results/mnist_2/save_model_VAE_f_share_test_32_4704.0_0.1_lr_0.0100_80_0.5000_wd_0.0005_2023-06-29_12-34-46'
        # self.eval_path = '/home/ruxin2/PPA-classification-prediction/Causality/results/mnist_2/save_model_VAE_f_share_test_32_4704.0_0.1_lr_0.0100_80_0.5000_wd_0.0005_2022-12-02_22-58-39'
...
print('begin fix s')
# 1300-> 1, 4400->0, 3700->7
# instance_num = [4400, 6600, 9250]
instance_num = [6600]
instance_num = [9249, 8125,5144, 3584]
...
for dim in range(6, 9):
    ...
    for i in range(z_init.shape[0]):
        if i in instance_num:
            ...
            print("raw:", np.max(img_list[0]), np.min(img_list[0]))
            print("5", np.max(img_list[5]), np.min(img_list[5]))
            plot_img = np.concatenate(img_list, axis=1)
            # plt.imshow(plot_img)
            if not os.path.exists('./CB_923_5_selection_bias_saved_img_fix_s-4/'):
                os.makedirs('./CB_923_5_selection_bias_saved_img_fix_s-4/')
            plt.imsave('./CB_923_5_selection_bias_saved_img_fix_s-4/fix_s_dim%03d_%05d.png' % (
                dim, i), plot_img)
...
for dim in range(10, 14):
    # dim = 8
    idx = dim
    max_val, min_val = max(s[:, idxs]), min(s[:, idxs])
    delta = (max_val - min_val) / (nums-1)
    for i in range(z_init.shape[0]):
        if i in instance_num:
            #print("i:", i)
            ...
            img_list = [raw_x]
            for ii in range(10,16):
                s[i, dim] = min_val + delta * (ii)
                img_list.append(model.decode_x(torch.cat([torch.from_numpy(z[i, :]).unsqueeze(0).cuda().float(),
                                                          torch.from_numpy(s[i, :]).unsqueeze(0).cuda().float()], dim=1))[:, :, 2:30, 2:30].contiguous().squeeze(0).detach().permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5)
            plot_img = np.concatenate(img_list, axis=1)
            # plt.imshow(plot_img)
            if not os.path.exists('./CB_923_5_selection_bias_saved_img_fix_z-4/'):
                os.makedirs('./CB_923_5_selection_bias_saved_img_fix_z-4/')
            plt.imsave('./CB_923_5_selection_bias_saved_img_fix_z-4/fix_z_dim%03d_%05d.png' %
                       (dim, i), plot_img)
