...
def evaluate(epoch, model, dataloader, args):
    ...
    for batch_idx, (x, target, env, u) in enumerate(dataloader):
        if x.size(0) == 1:
            ...
            plt.axis('off')
            if not os.path.exists('./ERM_2_confounding_bias_saved_img_grdient_NICO_train/'):
                os.makedirs('./ERM_2_confounding_bias_saved_img_grdient_NICO_train/')
            plt.savefig('./ERM_2_confounding_bias_saved_img_grdient_NICO_train/%03d.png' %
                        (batch_begin))
