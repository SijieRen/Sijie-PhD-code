...
def main():
    ...
    front_params = {}
    # front model cluster env num
    front_params['num_clusters'] = args.env_num

    back_params = {}
    back_params['input_dim'] = input_dim  # x
    back_params['output_dim'] = args.num_classes  # pred_y
    back_params['sigma'] = args.sigma  # 0.1
    back_params['lam'] = args.lambd  # 0.1
    back_params['alpha'] = args.alpha  # 1000.0
    back_params['hard_sum'] = args.hard_sum  # 10
    back_params['overall_threshold'] = args.overall_threshold  # 0.20
    whole_iters = args.epochs
    model = HRM(front_params, back_params, args)

    # pytorch_total_params_front = sum(p.numel()
    #                                  for p in model.frontend.model.parameters())
    pytorch_total_params_back = sum(
        p.numel() for p in model.backend.backmodel.parameters()) * 2  # todo *2 keyi shishi
    print('model params: ', pytorch_total_params_back,
          '%0.4f M' % (pytorch_total_params_back / 1e6))

    # if args.optimizer == 'sgd':
    #     optimizer = optim.SGD(model.backend.backmodel.parameters().parameters(), lr=args.lr,
    #                           momentum=args.momentum, weight_decay=args.reg)
    # else:
    #     optimizer = optim.Adam(
    #         model.backend.backmodel.parameters().parameters(), lr=args.lr, weight_decay=args.reg)

    # pytorch_total_params = sum(p.numel() for p in model.parameters())
    # print('model params: ', pytorch_total_params,
    #       '%0.4f M' % (pytorch_total_params / 1e6))
    # todo train test hanshu  xuyao chongxie
    all_weights = torch.tensor(np.zeros(input_dim, dtype=np.float32)).cuda()
    result_weight = model.solve(whole_iters, train_loader)
    all_weights += result_weight.cuda()
    # print("result_weight.cuda(): ", result_weight.size(), result_weight)
    logger.info("*** Start testing!!! ***")
    mask = torch.where(result_weight > back_params['overall_threshold'])
    # print("mask: ", mask.size(), mask[0].size())
    # print(torch.sum(result_weight))
    # print(torch.sum(mask[0]), torch.sum(mask[1]), torch.sum(mask[2]))
    # print(result_weight > back_params['overall_threshold'])
    _, test_acc = model.test(test_loader, result_weight,
                             back_params['overall_threshold'])
    best_acc = -1
    # for epoch in range(1, args.epochs + 1):
    #     adjust_learning_rate(optimizer, epoch, args.lr,
    #                          args.lr_decay, args.lr_controler)
    #     _, _ = train(epoch, model.backend.backmodel, optimizer,
    #                  train_loader, test_loader, args)
    #     if val_loader is not None:
    #         pred_val, val_acc = evaluate(
    #             model.backend.backmodel, val_loader, args)
    #     else:
    #         pred_val = None
    #         val_acc = -1
    #     pred_test, test_acc = evaluate(
    #         model.backend.backmodel, test_loader, args)
    if test_acc >= best_acc:
        best_acc = copy.deepcopy(test_acc)
        best_acc_ep = copy.deepcopy(args.epochs)
        is_best = 1
    #     else:
    #         is_best = 0
    #     other_info = {
    ...
    logger.info("current test_acc: %0.4f, best_acc: %0.4f, at ep %d"
                % (test_acc, best_acc, best_acc_ep))
