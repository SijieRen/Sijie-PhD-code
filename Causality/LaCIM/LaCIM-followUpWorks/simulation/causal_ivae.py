...
class Causal_VAE(nn.Module):
    def __init__(self, dim_z, dim_s, dim_x, dim_y, prior=None, decoder=None, encoder=None,
        ...
        # decoder x params
        self.fx = MLP(dim_z+dim_s, dim_x, hidden_dim, n_layers,
                      activation=activation, slope=slope)
        self.decoder_x_var = MLP(
            dim_z+dim_s, dim_x, hidden_dim, n_layers, activation=activation, slope=slope)
        # decode y params
        self.fy = MLP(dim_s, dim_y, hidden_dim, n_layers,
                      activation=activation, slope=slope)
        self.decoder_y_var = MLP(
            dim_s, dim_y, hidden_dim, n_layers, activation=activation, slope=slope)
        # encoder params
        self.g = MLP(dim_x+dim_y, dim_z+dim_s, hidden_dim,
                     n_layers, activation=activation, slope=slope)
        self.logv = MLP(dim_x+dim_y, dim_z+dim_s, hidden_dim,
                        n_layers, activation=activation, slope=slope)
    ...
    def elbo(self, x, y):
        decoder_x_params, decoder_y_params, (g, v), z, s, prior_params = self.forward(
            x, y)
        zs = torch.cat((z, s), 1)
...
class Causal_iVAE(nn.Module):
    def __init__(self, dim_z, dim_s, dim_x, dim_y, dim_c, prior=None, decoder=None, encoder=None,
        ...
        # prior_params
        self.prior_mean = MLP(dim_c, dim_s+dim_z, hidden_dim,
                              n_layers, activation=activation, slope=slope)
        # print('dim_c', dim_c,'self.prior_mean',self.prior_mean)
        self.logl = MLP(dim_c, dim_s+dim_z, hidden_dim,
                        n_layers, activation=activation, slope=slope)
        # decoder x params
        self.fx = MLP(dim_z+dim_s, dim_x, hidden_dim, n_layers,
                      activation=activation, slope=slope)
        ...
        self.decoder_y_var = MLP(
            dim_s, dim_y, hidden_dim, n_layers, activation=activation, slope=slope)
        # encoder params
        self.g = MLP(dim_x+dim_y+dim_c, dim_z+dim_s, hidden_dim,
                     n_layers, activation=activation, slope=slope)
        self.logv = MLP(dim_x+dim_y+dim_c, dim_z+dim_s, hidden_dim,
                        n_layers, activation=activation, slope=slope)
