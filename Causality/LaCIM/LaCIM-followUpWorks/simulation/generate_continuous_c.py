    ...
    for i in range(num_cluster):
        if i == 0:
            ...
            c = np.random.multivariate_normal(
                K_mu[i, :], sigma, n_per_clus)  # every sample has a c
        else:
            label = np.r_[label, i*np.ones(n_per_clus,)]
            c = np.r_[c, np.random.multivariate_normal(
                K_mu[i, :], sigma, n_per_clus)]  # every sample has a c
...
def save_data(num_cluster, n_per_clus, dim_c, dim_z, dim_s, dim_x, dim_y, n_layers, activation, slope=0.5, K_mu=None, dtype=np.float32, seed=None):
    ...
    path = '/home/ruxin2/nips2021_zhuankan_data_CB/'
    path += str(n_per_clus)
    path += '_' + str(dim_c)
    path += '_' + str(dim_z)
    path += '_' + str(dim_s)
    ...
    c, label, K_mu = generate_c(
        num_cluster, n_per_clus, dim_c, dim_s, K_mu, seed, dtype)
    label = to_one_hot(label, num_cluster)[0].astype(dtype)
    z, s, mu_z, sigma_z, mu_s, sigma_s = generate_zs(
        c, K_mu, label, dim_z, dim_s, seed)
