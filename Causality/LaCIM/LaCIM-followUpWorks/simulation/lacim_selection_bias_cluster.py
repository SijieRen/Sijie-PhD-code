...
while i < 5:
    ...
    if if_plot:
        ...
        # plt.show()
        plt.savefig('./SB_simu_img/pic_lacim_{}_i_{}.png'.format("gt_s", i))
    ...
    if if_plot:
        ...
        ax = plt.axes()
        # ax.set_facecolor("FloralWhite")
        plt.scatter(z_gt[c_gt.argmax(dim=1) == 0, 1],
                    z_gt[c_gt.argmax(dim=1) == 0, 0], c=color[0])
        plt.scatter(z_gt[c_gt.argmax(dim=1) == 1, 1],
                    z_gt[c_gt.argmax(dim=1) == 1, 0], c=color[1])
        plt.scatter(z_gt[c_gt.argmax(dim=1) == 2, 1],
                    z_gt[c_gt.argmax(dim=1) == 2, 0], c=color[2])
        plt.scatter(z_gt[c_gt.argmax(dim=1) == 3, 1],
                    z_gt[c_gt.argmax(dim=1) == 3, 0], c=color[3])
        plt.scatter(z_gt[c_gt.argmax(dim=1) == 4, 1],
                    z_gt[c_gt.argmax(dim=1) == 4, 0], c=color[4])
        # plt.show()
        plt.savefig('./SB_simu_img/pic_lacim_{}_i_{}.png'.format("gt_z", i))
    ...
    j = 0
    while j < 10:
        model_ivae = Causal_iVAE(
            dim_z, dim_s, dim_x, dim_y, dim_c, device=device)
        ...
        for k in range(2000):
            ...
            elbo, elbo_1, elbo_2, elbo_3, z, s, decoder_x_params, decoder_y_params, prior_params = model_ivae.elbo(
                x, y, c)
            # print(elbo)
            elbo.backward()
            optimizer_ivae.step()
            ...
            if torch.sum(torch.isnan(z_ivae)) == 0 and torch.sum(torch.isnan(s_ivae)) == 0:
                perf_z = mean_corr_coef(
                    z_gt.cpu().numpy(), z_ivae.detach().cpu().numpy())
                perf_s = mean_corr_coef(
                    s_gt.cpu().numpy(), s_ivae.detach().cpu().numpy())
        ...
        if if_plot:
            ...
            ax = plt.axes()
            # ax.set_facecolor("FloralWhite")
            plt.scatter(s_ivae_plot[c_gt.argmax(dim=1) == 0, 1],
                        s_ivae_plot[c_gt.argmax(dim=1) == 0, 0], c=color[0])
            plt.scatter(s_ivae_plot[c_gt.argmax(dim=1) == 1, 1],
                        s_ivae_plot[c_gt.argmax(dim=1) == 1, 0], c=color[1])
            plt.scatter(s_ivae_plot[c_gt.argmax(dim=1) == 2, 1],
                        s_ivae_plot[c_gt.argmax(dim=1) == 2, 0], c=color[2])
            plt.scatter(s_ivae_plot[c_gt.argmax(dim=1) == 3, 1],
                        s_ivae_plot[c_gt.argmax(dim=1) == 3, 0], c=color[3])
            plt.scatter(s_ivae_plot[c_gt.argmax(dim=1) == 4, 1],
                        s_ivae_plot[c_gt.argmax(dim=1) == 4, 0], c=color[4])
            # plt.show()
            plt.savefig(
                './SB_simu_img/pic_lacim_{}_i_{}_j_{}.png'.format("s_ivae", i, j))
        ...
        if if_plot:
            ...
            ax = plt.axes()
            # ax.set_facecolor("FloralWhite")
            plt.scatter(z_ivae_plot[c_gt.argmax(dim=1) == 0, 1],
                        z_ivae_plot[c_gt.argmax(dim=1) == 0, 0], c=color[0])
            plt.scatter(z_ivae_plot[c_gt.argmax(dim=1) == 1, 1],
                        z_ivae_plot[c_gt.argmax(dim=1) == 1, 0], c=color[1])
            plt.scatter(z_ivae_plot[c_gt.argmax(dim=1) == 2, 1],
                        z_ivae_plot[c_gt.argmax(dim=1) == 2, 0], c=color[2])
            plt.scatter(z_ivae_plot[c_gt.argmax(dim=1) == 3, 1],
                        z_ivae_plot[c_gt.argmax(dim=1) == 3, 0], c=color[3])
            plt.scatter(z_ivae_plot[c_gt.argmax(dim=1) == 4, 1],
                        z_ivae_plot[c_gt.argmax(dim=1) == 4, 0], c=color[4])
            # plt.show()
            plt.savefig(
                './SB_simu_img/pic_lacim_{}_i_{}_j_{}.png'.format("z_ivae", i, j))
            print("*** Finish ploting lacim_z_ivae _i_{}_j_{} !!! ***".format(i, j))
        perf_ivae_z = mean_corr_coef(
            z_gt.cpu().numpy(), z_ivae.detach().cpu().numpy())
        perf_ivae_s = mean_corr_coef(
            s_gt.cpu().numpy(), s_ivae.detach().cpu().numpy())
    ...
    with open(path[:-6]+'_%s' % (os.path.basename(__file__)[:-3]) + '/lacim_final.pkl', 'wb') as f:
        dicts = {
            'i': i, 'j': j, 'k': k,
            'z_perf_ivae_discrete_3': z_perf_ivae_discrete_3,
            's_perf_ivae_discrete_3': s_perf_ivae_discrete_3,
    ...
    while j < 10:
        ...
        if if_plot:
            ...
            # plt.show()
            plt.savefig(
                './SB_simu_img/pic_poollacim_{}_i_{}_j_{}.png'.format("s_vae", i, j))
        ...
        if if_plot:
            ...
            ax = plt.axes()
            # ax.set_facecolor("FloralWhite")
            plt.scatter(z_vae_plot[c_gt.argmax(dim=1) == 0, 1],
                        z_vae_plot[c_gt.argmax(dim=1) == 0, 0], c=color[0])
            plt.scatter(z_vae_plot[c_gt.argmax(dim=1) == 1, 1],
                        z_vae_plot[c_gt.argmax(dim=1) == 1, 0], c=color[1])
            plt.scatter(z_vae_plot[c_gt.argmax(dim=1) == 2, 1],
                        z_vae_plot[c_gt.argmax(dim=1) == 2, 0], c=color[2])
            plt.scatter(z_vae_plot[c_gt.argmax(dim=1) == 3, 1],
                        z_vae_plot[c_gt.argmax(dim=1) == 3, 0], c=color[3])
            plt.scatter(z_vae_plot[c_gt.argmax(dim=1) == 4, 1],
                        z_vae_plot[c_gt.argmax(dim=1) == 4, 0], c=color[4])
            # plt.show()
            plt.savefig(
                './SB_simu_img/pic_poollacim_{}_i_{}_j_{}.png'.format("z_vae", i, j))
