...
import sys
from NIRM.models.LaCIM_model import IVAE_2D_MNIST, IVAE_2D_NICO, IVAE_3D_AD
...
class iCaRL_cmnist(nn.Module):
    def __init__(self,
                 in_channel,
                 ):
        super(iCaRL_cmnist, self).__init__()
        self.in_channel = in_channel

        self.Linear_prior = nn.Sequential(
            nn.Linear(self.in_dim, 2)
        )

        self.NonLinear_prior = nn.Sequential(
    ...
    def Conv_bn(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1,
                bias=True, groups=1):
        layer = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size,
                      stride=stride, padding=padding, bias=bias, groups=groups),
    ...
    def Fc_bn(self, in_channels, out_channels):
        layer = nn.Sequential(
            nn.Linear(in_channels, out_channels),
            nn.BatchNorm1d(out_channels))
        return layer
