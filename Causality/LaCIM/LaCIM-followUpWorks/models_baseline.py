...
class sVAE(nn.Module):  # sijie sVAE zhuankan ADNI
    def __init__(self,
                 in_channel=1,
                 u_dim=2,
                 us_dim=6,
    ...
    def forward(self, x, env, us, feature=0, is_train=1):
        if is_train == 1:
            mu, logvar = self.encode(x, env, us)
            mu_prior, logvar_piror = self.encode_prior(x, env)
            z = self.reparametrize(mu, logvar)
            rec_x = self.Dec_x(z)
            pred_y = self.Dec_y(z)
            if feature == 1:
            ...
            with torch.no_grad():
                ...
                for env_idx in range(self.args.env_num):
                    mu, logvar = self.encode(x, env_idx, us)
                    z = self.reparametrize(mu, logvar)
                    # if self.decoder_type == 0:  # 0 -> nico
                    recon_x = self.Dec_x(z)
                    # else:  # 1 -> cmnist
                    #     rec_x = self.Dec_x(z)[:, :, 2:30, 2:30].contiguous()
...
class Generative_model_f_2D_unpooled_env_t_AD(nn.Module):  # sijie
    def __init__(self,
                 in_channel=1,
                 u_dim=2,
                 us_dim=6,
            ...
            with torch.no_grad():
                ...
                for env_idx in range(self.args.env_num):
                    ...
                    for ss in range(self.args.sample_num):
                        ...
                        if z_init is None:
                            z_init, s_init = z, s
                            min_rec_loss = F.binary_cross_entropy(recon_x.view(-1, 1 * self.args.crop_size ** 2),
                                                                  (raw_x * 0.5 + 0.5).view(-1,
                                                                                           1 * self.args.crop_size ** 2),
                                                                  reduction='none').mean(1)
                        else:
                            new_loss = F.binary_cross_entropy(recon_x.view(-1, 1 * self.args.crop_size ** 2),
                                                              (raw_x * 0.5 + 0.5).view(-1,
                                                                                       1 * self.args.crop_size ** 2),
                                                              reduction='none').mean(1)
                            for i in range(x.size(0)):
                ...
                for env_idx in range(self.args.env_num):
                    ...
                    for ss in range(self.args.sample_num):
                        ...
                        if z_init is None:  # sijiez_init, s_init = None, None
                            ...
                            if self.args.mse_loss:  # sijie mse_loss=1
                                new_loss = F.mse_loss(recon_x.view(-1, 3 * sel  .args.image_size ** 2),
                                                      (raw_x * 0.5 + 0.5).view(-1,
                                                                               3 * self.args.image_size ** 2),
                                                      reduction='none').mean(1)
                            else:
...
class sVAE_2D(nn.Module):
    ...
    def forward(self, x, env, us, feature=0):
        mu, logvar = self.encode(x, env, us)
        mu_prior, logvar_piror = self.encode_prior(env, us)
        z = self.reparametrize(mu, logvar)
        rec_x = self.Dec_x(z)
        pred_y = self.Dec_y(z)
        if feature == 1:
...
class sVAE_f_2D(nn.Module):  # sijie sVAE zhuankan 2D
    def __init__(self,
                 in_channel=1,
                 u_dim=2,
                 us_dim=6,
            ...
            with torch.no_grad():
                ...
                for env_idx in range(self.args.env_num):
                    ...
                    if self.decoder_type == 0:  # 0 -> nico
                        ...
                        recon_x = self.Dec_x(z)[:, :, 2:30, 2:30].contiguous()
                    if z_init is None:  # sijiez_init, s_init = None, None
                        z_init = z
                        min_rec_loss = F.mse_loss(recon_x.view(-1, 3 * self.args.image_size ** 2),
                                                  (raw_x).view(-1,
