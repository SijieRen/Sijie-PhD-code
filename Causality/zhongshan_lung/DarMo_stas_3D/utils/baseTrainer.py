...
from data.STAS_dataloader import Dataloader_3D
...
from models.Maintrainer_stas import Model_STAS_3D, Model_STAS_3D_bs
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.backbone: #TODO modify the 3D model
        model = Model_STAS_3D(1, 64, "adjacency_matrix.pkl", args)
    else:
        model = Model_STAS_3D_bs(1, 64, 2, args.if_mmf)
    ...
    elif args.crop_size == 80:
        dataset_dir = "data/STAS_dataset_3D_80-80-80.xls"
    elif args.crop_size ==32:
        dataset_dir = "data/STAS_dataset_3D-bgMAX_maskXYZgezi_padding0.xls"
    
    if args.spacing == "050505":
        dataset_dir = "data/STAS_dataset_3D-bgMAX_maskXYZgezi_padding0_stdMean_noResample-check2.xls"
    elif args.spacing == "111":
        dataset_dir = "data/STAS_dataset_3D-bgMAX_maskXYZgezi_padding0_stdMean_noResample_testSpaceing111-check.xls"
    

    if args.if_randomTest:
        if args.spacing == "050505":
            dataset_dir = "data/STAS_dataset_3D-bgMAX_maskXYZgezi_padding0_stdMean_noResample-check2-random82.xls"
        elif args.spacing == "111":
            dataset_dir = "data/STAS_dataset_3D-bgMAX_maskXYZgezi_padding0_stdMean_noResample_testSpaceing111-check-random82.xls"
        # dataset_dir = "data/STAS_dataset_3D_80-80-80.xls"
        """
        XYA_max: max mask 坐标切割 padding 灰度均值
        XYZ——gezi padding0: 各方向按照XYZmask适应坐标切割，结果为不均等3d np数据
        bgMAX_maskXYZgezi_padding0: 背景按照mask max，mask 病灶按照XYZ自适应
        """
