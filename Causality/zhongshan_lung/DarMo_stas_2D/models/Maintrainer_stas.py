...
from .vit_pytorch_main.vit_pytorch.vit import ViT
from .vit_pytorch_main.vit_pytorch.vit_d import ViT_d
...
class Model_STAS_2D_bs(Basenet): 
    ...
    def __init__(self,
                 ...
                 args = None ,
                 **kwargs) -> None:
        super(Model_STAS_2D_bs, self).__init__()
        ...
        if self.if_mmf:
            self.mlp = nn.Sequential(self.Fc_bn_ReLU(2, 128),
...
class Model_STAS_2D(Basenet): 
    ...
    def __init__(self,
        ...
        #                     nn.Tanh())
        # self.cls_fc = nn.Sequential(self.Fc_bn_ReLU(self.latent_dim//2, 128),
        #                            nn.Linear(128, 2),
        #                            )
        ...
        self.enc_A1_x = nn.Sequential(self.Fc_bn_ReLU(self.latent_dim*2, 128), 
                                    self.Fc_bn_ReLU(128, 256),
        ...
        # self.SFM = nn.Sequential(nn.Softmax(dim=1),)
    ...
    def encode_x(self, input: Tensor, A1: Tensor) -> List[Tensor]:
        ...
        A1 = self.enc_A1(A1)  
        v = self.enc_A1_x(torch.cat((v, A1), dim=1))
        # result = torch.flatten(result, start_dim=1)

        # Split the result into mu and var components
        # of the latent Gaussian distribution
        mu = self.fc_mu(v)
        log_var = self.fc_var(v)
    ...
    def forward(self, input: Tensor, m_id: int, h_id: int, A1: Tensor, gcn_target: Tensor, criterion_gcn, state: str, **kwargs) -> List[Tensor]:
        ...
        if 'train' in state:
            ...
            if "trans" in args.if_transformer:
                ...
                d = self.Resnetencoder2(input, str(h_id))
    ...
    def loss_function(self,
        ...
        if self.args.rec_loss_check == "bce":
            recons_loss = F.binary_cross_entropy(recons.view(-1, 32 ** 3), input.view(-1, 32 ** 3), reduction='mean')
        ...
        loss = self.args.para_recon * recons_loss + self.args.para_kld * kld_loss
    ...
    def decode_x(self):
        return nn.Sequential(
            ...
            self.Conv_bn_ReLU(in_channels=64, out_channels=64,
                              kernel_size=3, stride=1, padding=1),
            self.TConv_bn_ReLU(in_channels=64, out_channels=32, # *2^4
                               kernel_size=2, stride=2, padding=0),
