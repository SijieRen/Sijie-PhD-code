    ...
    for i, (input, target, A_1, gcn_target, m_id, h_id) in enumerate(train_loader): #TODO modify the dataloader
        ...
        for machine in range(m):
            ...
            if input_var[m_id==machine,:,:,:].size(0) > 1:
                # print("i: ",i, input_var[m_id==machine,:,:,:].size(0))
                # print("input_var[m_id==machine,:,:,:]", input_var[m_id==machine,:,:,:].size())
                # print("A_1[m_id==machine,:]", A_1[m_id==machine,:].size()) #.unsqueeze(0)
                # print("gcn_target_var", gcn_target_var[m_id==machine,:].size())
                if input_var[m_id==machine,:,:,:].size(0) ==1:
                    output = model(input_var[m_id==machine,:,:,:], machine, 0, A_1[m_id==machine,:], gcn_target_var[m_id==machine,:], criterion_gcn,'train')
                else:
                    output = model(input_var[m_id==machine,:,:,:], machine, 0, A_1[m_id==machine,:], gcn_target_var[m_id==machine,:], criterion_gcn,'train')
                ...
                loss = loss_vae_dict['loss'] + \
                            int(args.para_cls) * loss_cls + \
                                int(args.para_gcn) *loss_gcn
                                    # int(args.para_recon)*loss_rec
                ...
                losses_recon.update(loss_vae_dict['Reconstruction_Loss'].item(), input[m_id==machine,:,:,:].size(0))
                losses_kl.update(loss_vae_dict['KLD'].item(), input[m_id==machine,:,:,:].size(0))
            ...
            loss_count += input_var[m_id==machine,:,:,:].size(0)
