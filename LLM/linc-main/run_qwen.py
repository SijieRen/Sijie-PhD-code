...
from eval.tasks import ALL_TASKS, TASK_FOLIO
...
def main():
    ...
    elif "folio" in args.tasks:
        task_names = TASK_FOLIO
    else:
        task_names = set()
    ...
    if args.generations_path:
        ...
        # for task in task_names:
        #     print("format(task) + args.save_generations_raw_path", "{}", str(args.save_generations_raw_path) + task + ".json")
        ...
        print("Taskname: *********************", task_names)

        for task in task_names:
            print("Task ********************* Task", str(task))
            # print("from pathlib import Path", Path(str(task)) / args.save_references_path)
            # print("str(task) + args.save_references_path.as_posix()", str(task) + args.save_references_path.as_posix())
            if args.generation_only:
                ...
                if accelerator.is_main_process:
                    if args.save_generations_raw:
                        # str_save_generations_raw = task + "_" + args.save_generations_raw_path
                        with open(str(args.save_generations_raw_path) + task + ".jsonl", "w") as fp:
                            json.dump(generations_raw, fp)
                            print("raw generations were saved")
                    if args.save_generations_prc:
                        with open(str(args.save_generations_prc_path) + task + ".jsonl", "w") as fp:
                            json.dump(generations_prc, fp)
                            print("processed generations were saved")
                    if args.save_references:
                        with open(str(args.save_references_path) + task + ".jsonl", "w") as fp:
