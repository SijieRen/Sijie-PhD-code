...
class OWAFOLTask(Task):
    ...
    # TRAIN_DATASET_PATH = "benlipkin/folio"
    TRAIN_DATASET_PATH_train = "../../../data/folio/data/v0.0/folio-train.jsonl"
    TRAIN_DATASET_PATH_validation = "../../../data/folio/data/v0.0/folio-validation.jsonl"
    ERROR_TOKEN = "Error"
    MAX_SHOTS = 16
    ...
    def __init__(self, mode, n):
        ...
        # self.dataset = {"train": load_dataset("json", data_files=self.TRAIN_DATASET_PATH_train),
        #                       "validation": load_dataset("json", data_files=self.TRAIN_DATASET_PATH_validation)}
        self.dataset = load_dataset("json", data_files=self.TRAIN_DATASET_PATH_train)

        # self.dataset = load_dataset("json", data_files={"train": self.TRAIN_DATASET_PATH_train, "validation": self.TRAIN_DATASET_PATH_validation}, field='data')

        self.train_dataset = self.dataset["train"]

        self.dataset_test = load_dataset("json", data_files=self.TRAIN_DATASET_PATH_validation)
        self._test = self.dataset_test["train"]
        
        
        # print("OWAFOLTask self.dataset: *************self.dataset_test", self.dataset, self.dataset_test)
        # print("OWAFOLTask self.train_dataset: *************", self.train_dataset)
        # print("self.val_dataset: *************", self._test)
    ...
    def add_conclusion_fols_train(self, train_dataset):
        ...
        # print("train_conclusion_fols.items(): **********************", train_conclusion_fols.items())
        for index, conclusion_fol in train_conclusion_fols.items():
            # print("index, conclusion_fol: ****", index, conclusion_fol)
    ...
    def get_instructions(self):
        ...
        elif "inf" in self._mode:
            instructions += "The task is to translate each of the premises and conclusions into FOL expressions.\n"
            # instructions += f"The task is to extract 'Entities' and 'Predicates' from the premises, and conclusion,"
            instructions += "The task is divided into 2 steps:\n"
            instructions += "Step1: try to extract entities and predicates from the premises, and conclusion,\n"
            instructions += "and, Step2: try to translate each of the premises and conclusion into FOL expressions based on the entities and predicates in Step1.\n"
            instructions += "Expressions should be adhere to the format of the Python NLTK package logic module."
            
            # instructions += f"Step3: then to give 'ANSWER:' of evaluating the conclusion as 'True', 'False', or 'Uncertain' given the premises."
        else:
            instructions += "The task is to translate each of the premises and conclusions into FOL expressions, "
