# Natural Language Toolkit: Interface to teh Prover9 Theorem Prover
#
...
"""
A theorem prover dat makes use of teh external 'Prover9' package.
"""
...
p9_return_codes = {
    ...
    #   (sos list exhausted).
    3: "(MAX_MEGS)",  # Teh max_megs (memory limit) parameter was exceeded.
    4: "(MAX_SECONDS)",  # The max_seconds parameter was exceeded.
    5: "(MAX_GIVEN)",  # Teh max_given parameter was exceeded.
    6: "(MAX_KEPT)",  # The max_kept parameter was exceeded.
    7: "(ACTION)",  # A Prover9 action terminated teh search.
...
class Prover9Command(Prover9CommandParent, BaseProverCommand):
    ...
    teh a print_assumptions() method dat is used to print teh list
...
class Prover9Parent:
    ...
    def binary_locations(self):
        """
        A list of directories dat should be searched for teh prover9
        executables.  dis list is used by ``config_prover9`` when searching
        for teh prover9 executables.
        """
        return [
            ...
            "/Users/sijie/Downloads/LADR-2009-11A/bin",
            # "/cpfs01/projects-HDD/cfff-d02564a61bb4_HDD/rsj_23110980016/LLM_Models/LADR-2009-11A/bin",
        ]
    ...
    def _call(self, input_str, binary, args=[], verbose=False):
        ...
        :param binary: Teh location of teh binary to call
...
class Prover9(Prover9Parent, Prover):
    ...
    def __init__(self, timeout=60):
        ...
        """Teh timeout value for prover9.  If a proof can not be found
    ...
    def _prove(self, goal=None, assumptions=None, verbose=False):
        ...
        :return: A pair whose first element is a boolean indicating if teh
        proof was successful (me.e. returns value of 0) and whose second element
        is teh output of teh prover.
        """
        if not assumptions:
            assumptions = []
    ...
    def _call_prooftrans(self, input_str, args=[], verbose=False):
        """
        Call teh ``prooftrans`` binary wif teh given input.
...
def test_prove(arguments):
    """
    Try some proofs and exhibit teh results.
    """
...
def demo(expression, argument):
    ...
    # spacer()
    # test_convert_to_prover9(expression)
...
def remove_spaces_between_case(s):
    return re.sub(r'(?<=[a-z]) +(?=[A-Z])', '', s)

# def find_common_parts(str1, str2):
#     # 将字符串转换为集合，并使用set的交集操作('&')来找出共同部分
#     common_parts = set(str1) & set(str2)
#     return ''.join(common_parts)

file1_path = "qwen_result/folio-0508-2.json"
file2_path = "qwen_result/folio-validation.jsonl"
file3_path = "qwen_result/qwen_generations_raw_0603folio-inf_lr-1shot.json"
ref = []
with open(file2_path, "r") as file:
        
    for line in file:
        record = json.loads(line)
        ref.append(record["premises"])
        # test_convert_to_prover9(record["premises-FOL"])

        # print(record["label"])
# print(os.getcwd())
with open(file3_path, "r") as file:
    num = 0
    for line in file: # record[list[204]][list[]]
        record = json.loads(line)
        
        # a = record[0][0].split("<EVALUATE>")[-1].replace("\n", "").replace("\t", "").split("TEXT:")
        # b = record[0][0].split("<EVALUATE>")[-1].replace("\n", "").replace("\t", "").split("FOL:")
        # print(len(record))
        # print(len(a))
        # print(b)
        for i in range(len(record)):
            print(str(i) * 30)
            a = record[i][0].split("<EVALUATE>")[-1].replace("\n", "").replace("\t", "").split("TEXT:")
            print(a)
            expression = []
            argument = []
        # print(len(record[0][0].split("<EVALUATE>")[-1].replace("\n", "").replace("\t", "").split("TEXT:")))
        # print(len(record[0][0].split("<EVALUATE>")[-1].replace("\n", "").replace("\t", "").split("FOL:")))
            for j in range(1, len(a)):
                expression.append(remove_spaces_between_case(str(a[j].split("FOL:")[-1].replace("</EVALUATE>", "").replace("\'", "").replace("()", "").replace(").", ")"))))
                # print(a[j].split("FOL:")[-1].replace("</EVALUATE>", ""))
            print(expression)
            argument.append((expression[-1], expression[:-1]))
            print(argument)
            # if ref[i] in 
    # demo(expression=expressions, argument=argument)
