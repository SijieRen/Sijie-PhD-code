...
def evaluate(args,
    ...
    pred_label_cur = np.zeros((len(test_loader.dataset), 1))
    with torch.no_grad():
        batch_begin = 0
        ...
        for batch_idx, (data_1, data_2, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(test_loader):
            ...
            pred_label_cur[batch_begin:batch_begin + data_1.size(0)] = P_current_1_i.argmax(dim=1,
                                                                                       keepdim=True).detach().cpu().numpy()
...
def main():
    ...
    args.load_dir = '../results/Ecur_10-31-modify/order1-ours-003/2020-10-31-22-50-13_Mixed_0.000200_60.000000_size_256_ep_120_0_R_Maculae/'
        ...
        for epoch in range(1, 1 + 1):
            ...
            for ss in range(len(val_loader_list)):
                ...
                val_results_list.append(
                    test_results_list[-1])
            train_results = test_results_list[-1]
