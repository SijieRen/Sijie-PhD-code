...
class Clas_ppa_train(data.Dataset):
    def __init__(self,
        ...
        if filename == 'std_all':
            ...
            print('load data std all pair')
            workbook1 = xlrd.open_workbook(
                r"../ppa-classi-dataset-onuse/ppa_07-02-order3-std-1-6all-8.xls")
    ...
    for batch_idx, (data_1, data_2, data_3, data_4, target_ppa, feature_1, feature_2, feature_3, feature_4,
        ...
        if batch_idx % 5 < args.D_epoch:
            ...
            for idx in range(data_1.size(0)):
                all_generated_feature = [G_net_list[0](feature=featuremap_1[idx].unsqueeze(0),
                                                      z=torch.cat([z[idx].unsqueeze(0),feature_1[idx].unsqueeze(0).view(1, -1, 1, 1)], dim=1),
                                                      grad_1=grad_1[idx].detach().cpu().numpy() - 1,
                                                      grad_2=grad_2[idx].detach().cpu().numpy() - 1)]
                all_generated_feature.append(G_net_list[0](feature=featuremap_2[idx].unsqueeze(0),
                                                      z=torch.cat([z[idx].unsqueeze(0),feature_2[idx].unsqueeze(0).view(1, -1, 1, 1)], dim=1),
                                                      grad_1=grad_2[idx].detach().cpu().numpy() - 1,
                                                      grad_2=grad_3[idx].detach().cpu().numpy() - 1))
                all_generated_feature.append(G_net_list[0](feature=featuremap_3[idx].unsqueeze(0),
                                                           z=torch.cat(
                                                               [z[idx].unsqueeze(0), feature_3[idx].unsqueeze(0).view(1, -1, 1, 1)],
                                                               dim=1),
                                                           grad_1=grad_3[idx].detach().cpu().numpy() - 1,
                                                           grad_2=grad_4[idx].detach().cpu().numpy() - 1))
                if idx == 0:
        ...
        if batch_idx % 5 >= args.D_epoch:
            ...
            for idx in range(data_1.size(0)):
                all_generated_feature = [G_net_list[0](feature=featuremap_1[idx].unsqueeze(0),
                                                       z=torch.cat([z[idx].unsqueeze(0),
                                                                    feature_1[idx].unsqueeze(0).view(1, -1,
                                                                                                     1, 1)],
                                                                   dim=1),
                                                       grad_1=grad_1[idx].detach().cpu().numpy() - 1,
                                                       grad_2=grad_2[idx].detach().cpu().numpy() - 1)]
                all_generated_feature.append(G_net_list[0](feature=featuremap_2[idx].unsqueeze(0),
                                                           z=torch.cat(
                                                               [z[idx].unsqueeze(0),
                                                                feature_2[idx].unsqueeze(0).view(1, -1, 1,
                                                                                                 1)],
                                                               dim=1),
                                                           grad_1=grad_2[idx].detach().cpu().numpy() - 1,
                                                           grad_2=grad_3[idx].detach().cpu().numpy() - 1))
                all_generated_feature.append(G_net_list[0](feature=featuremap_3[idx].unsqueeze(0),
                                                           z=torch.cat(
                                                               [z[idx].unsqueeze(0),
                                                                feature_3[idx].unsqueeze(0).view(1, -1, 1,
                                                                                                 1)],
                                                               dim=1),
                                                           grad_1=grad_3[idx].detach().cpu().numpy() - 1,
                                                           grad_2=grad_4[idx].detach().cpu().numpy() - 1))
...
def evaluate(args,
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data_1, data_2, data_3, data_4, target_ppa, feature_1, feature_2, feature_3, feature_4,
            ...
            for idx in range(data_1.size(0)):
                all_generated_feature = [G_net_list[0](feature=featuremap_1[idx].unsqueeze(0),
                                                      z=torch.cat([z[idx].unsqueeze(0), feature_1[idx].unsqueeze(0).view(1, -1, 1, 1)],
                                                                  dim=1),
                                                      grad_1=grad_1[idx].detach().cpu().numpy() - 1,
                                                      grad_2=grad_2[idx].detach().cpu().numpy() - 1)]
                all_generated_feature.append(G_net_list[0](feature=featuremap_2[idx].unsqueeze(0),
                                                           z=torch.cat(
                                                               [z[idx].unsqueeze(0), feature_2[idx].unsqueeze(0).view(1, -1, 1, 1)],
                                                               dim=1),
                                                           grad_1=grad_2[idx].detach().cpu().numpy() - 1,
                                                           grad_2=grad_3[idx].detach().cpu().numpy() - 1))
                all_generated_feature.append(G_net_list[0](feature=featuremap_3[idx].unsqueeze(0),
...
def save_results(args,
    ...
    # final pred(Without MA)
    args.logger.info('Final pred results(Without MA)')
    args.logger.info('Final pred results(Without MA)')
    desired_type = 'final'
    test_acc_mean = 0.0
    args.logger.info('best val acc epoch %d, val acc: %0.4f test acc:' % (args.best_val_acc_epoch, args.best_val_acc))
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            train_results = train(args,
                                  ...
                                  epoch)
            test_results_list = []
            val_results_list = []  # Ablation study: add judgement here(if hyperparameter: ......) 
