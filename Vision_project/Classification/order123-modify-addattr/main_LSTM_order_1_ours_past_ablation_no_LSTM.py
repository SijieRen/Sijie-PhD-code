    ...
    for batch_idx, (data_1, data_2, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(train_loader):
        ...
        if batch_idx % 5 < args.D_epoch:
            ...
            for p in model_2_generate.parameters():
                p.requires_grad = True
            for p in model_2_future.parameters():
                p.requires_grad = True
            ...
            for idx in range(data_1.size(0)):
                all_generated_feature = G_net(feature=featuremap_1[idx].unsqueeze(0),
                                              z=z1[idx].unsqueeze(0),
                                              att=feature_1[idx].view(1, -1, 1, 1),
                                              grad_1=grad_1[idx].detach().cpu().numpy() - 1,
                                              grad_2=grad_2[idx].detach().cpu().numpy() - 1)
                if idx == 0:
                    generate_feature_1 = all_generated_feature
                else:
                    generate_feature_1 = torch.cat([generate_feature_1,
                                                          all_generated_feature], dim=0)
            ...
            loss_future = torch.FloatTensor([0.0]).cuda()
            gen_count = 0
            loss_res_count = 0
            reg_mono_count = 0
            ...
            pred_future = model_2_future(featuremap_1, feature_1,
                                idx1=(grad_2 - grad_1 - 1).view(data_2.size(0), ).detach().cpu().numpy())
            loss_future_count = 0
            loss_cur_count = 0
            for i in range(data_1.size(0)):
                loss_future_count += 1
                loss_future = torch.add(loss_future, F.cross_entropy(pred_future[i].unsqueeze(0), target_ppa[i]))

                if target_ppa[i].detach().cpu().numpy()[0] == 0:
                    loss_cur_count += 2
                    loss_cur = torch.add(loss_cur,
                                         F.nll_loss(torch.log(eps + P_current_1_i[i].unsqueeze(0)), target_ppa[i]) + \
                                         F.nll_loss(torch.log(eps + P_current_1_j[i].unsqueeze(0)), target_ppa[i])
                                         )
                elif int(grad_1[i].detach().cpu().numpy()) == 1:
                    loss_cur_count += 1
                    loss_cur = torch.add(loss_cur,
                                         F.nll_loss(torch.log(eps + P_current_1_i[i].unsqueeze(0)), 1 - target_ppa[i]))

                if int(grad_2[i].detach().cpu().numpy()) == 6 and target_ppa[i].detach().cpu().numpy()[0] == 1:
                    loss_cur_count += 1
                    loss_cur = torch.add(loss_cur,
                                         F.nll_loss(torch.log(eps + P_current_1_j[i].unsqueeze(0)), target_ppa[i]))
            ...
            if loss_future_count != 0:
                loss_future = loss_future / loss_future_count
            if loss_cur_count != 0:
                loss_cur = loss_cur / loss_cur_count
            
            loss = loss_res + args.alpha * reg_mono + args.gamma * loss_gen + args.beta * loss_future + args.beta1 * loss_cur
            
            if loss.item() != 0:
                optimizer_M_2_generate.zero_grad()
                optimizer_M_2_res.zero_grad()
                optimizer_M_2_future.zero_grad()
                loss.backward(retain_graph=False)
                optimizer_M_2_generate.step()
                optimizer_M_2_res.step()
                optimizer_M_2_future.step()
        ...
        if batch_idx % 5 >= args.D_epoch:
            ...
            loss_temp = loss_res + args.alpha * reg_mono + args.gamma * loss_gen + args.beta * loss_future + args.beta1 * loss_cur
...
def evaluate(args,
    ...
    pred_result_cur = np.zeros((len(test_loader.dataset), args.class_num))
    pred_result_future = np.zeros((len(test_loader.dataset), args.class_num))
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data_1, data_2, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(test_loader):
            ...
            future_pred = torch.softmax(model_2_future(featuremap_1, feature_1.view(data_2.size(0), -1),
                                         idx1=(grad_2 - grad_1 - 1).view(data_2.size(0), ).detach().cpu().numpy()), dim=1)

            pred_result_cur[batch_begin:batch_begin + data_1.size(0), :] = P_current_1_i.detach().cpu().numpy()
            pred_result_future[batch_begin:batch_begin + data_1.size(0), :] = future_pred.detach().cpu().numpy()
            ...
            output_average_temp = (P_current_1_i[:, 1] + P_current_1_i[:, 0] * P_residue_1_i[:, 1] +
                                   gen_pred[:,1] + future_pred[:,1]) / 3.
    ...
    AUC_cur = sklearn.metrics.roc_auc_score(target, pred_result_cur[:, 1])
    acc_cur = sklearn.metrics.accuracy_score(target, np.argmax(pred_result_cur, axis=1))

    AUC_future = sklearn.metrics.roc_auc_score(target, pred_result_future[:, 1])
    ...
    results = {
        ...
        'pred_result_cur': pred_result_cur,
        'AUC_cur': AUC_cur,
        'acc_cur': acc_cur,

        'pred_result_future': pred_result_future,
        'AUC_future': AUC_future,
...
def save_results(args,
    ...
    if epoch == args.best_test_acc_epoch:
        ...
        torch.save(model_2_future.state_dict(), os.path.join(args.save_dir, 'best_test_acc' + '_model_2_future.pt'))
    ...
    if epoch == args.best_test_auc_epoch:
        ...
        torch.save(model_2_future.state_dict(), os.path.join(args.save_dir, 'best_test_auc' + '_model_2_future.pt'))
    ...
    if epoch == args.best_val_acc_epoch:
        ...
        torch.save(model_2_future.state_dict(), os.path.join(args.save_dir, 'best_val_acc' + '_model_2_future.pt'))
    ...
    if epoch == args.best_val_auc_epoch:
        ...
        torch.save(model_2_future.state_dict(), os.path.join(args.save_dir, 'best_val_auc' + '_model_2_future.pt'))
    ...
    if args.save_checkpoint > 0:
        save_checkpoint({
            ...
            'optimizer_M_2_future': optimizer_M_2_future.state_dict(),
...
def main():
    ...
    model_2_res = RN18_last_attr_ind(num_classes=args.class_num,
                                     ...
                                     concat_num=1,
                                     task_num=5).cuda()
    model_2_future = RN18_last_attr_ind(num_classes=args.class_num,
                                           feature_dim = len(args.feature_list),
                                           concat_num=1,
                                           task_num=5).cuda()
    ...
    if args.G_net_type == 'G_net':
        ...
        G_net = Generator_seq(feature_num=len(args.feature_list),
                              final_tanh=args.final_tanh,
                              in_channel=128).cuda()
        ...
        G_net = UNet(n_channels=128,
                     ...
                     feature_num=len(args.feature_list),
                     final_tanh=args.final_tanh,
    ...
    optimizer_M_2_future = optim.SGD([{'params': model_2_future.parameters(), 'lr': args.lr2,
                                    'weight_decay': args.wd2, 'momentum': args.momentum}
                                   ])
    ...
    try:
        evaluate(args, model_1, model_2_generate, model_2_future, model_2_res, G_net,
                     test_loader_list[0], 0)
        for epoch in range(1, args.epochs + 1):
            start_time = time.time()
            train_results = train(args, model_1, model_2_generate, model_2_future, model_2_res, G_net, D_net,
                                  train_loader, optimizer_M_2_generate, optimizer_M_2_res, optimizer_M_2_future, optimizer_G, optimizer_D,
                                  epoch)
            test_results_list = []
            val_results_list = []
            ...
            adjust_learning_rate(optimizer_M_2_future, epoch, args)
