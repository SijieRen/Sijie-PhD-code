...
def save_results_order3(args,
	...
	args.logger.info('mean test acc: %0.4f' % (test_acc_mean / 3))
	...
	args.logger.info('mean test auc: %0.4f' % (test_auc_mean / 3))
...
def train_order3(args,
	...
	for batch_idx, (data_1, data_2, data_3, data_4, target_ppa, feature_1, feature_2, feature_3, feature_4,
		...
		if batch_idx % 5 < args.D_epoch:
			...
			# y1 = torch.sign(y1)
			generate_feature_1, generate_feature_2, generate_feature_3, generate_feature_4,\
			y_hat1, y_hat2, y_hat3, y_hat4 = G_net1(z1, y1)
			
			y_hat1 = torch.sign(y_hat1).long()
			y_hat2 = torch.sign(y_hat2).long()
			y_hat3 = torch.sign(y_hat3).long()
			y_hat4 = torch.sign(y_hat4).long()
			...
			real_loss_4 = D_net(data_4).mean(0).view(1)
			fake_loss_4 = D_net(generate_feature_4).mean(0).view(1)
			
			loss_D = (real_loss_1 - fake_loss_1) + (real_loss_2 - fake_loss_2) + \
				(real_loss_3 - fake_loss_3) + (real_loss_4 - fake_loss_4)
			...
			for i in range(data_1.size(0)):
				R_num += 1
				num += 1
				deltaT = int(grad_4[i].detach().cpu().numpy()) - int(grad_1[i].detach().cpu().numpy())
				...
				loss_R_t01_1 += F.l1_loss(data_R_real, data_4[i].unsqueeze(0))
				loss_R_t01_2 += F.l1_loss(generate_feature_4[i].unsqueeze(0), data_R_gene)
			...
			for i in range(data_2.size(0)):
				R_num += 1
				num += 1
				deltaT = int(grad_4[i].detach().cpu().numpy()) - int(grad_2[i].detach().cpu().numpy())
				data_R_real = data_2[i].unsqueeze(0)
				data_R_gene = generate_feature_2[i].unsqueeze(0)
				for ii in range(deltaT):
					data_R_real = model_R(data_R_real)
					data_R_gene = model_R(data_R_gene)
				loss_R_t02_1 += F.l1_loss(data_R_real, data_4[i].unsqueeze(0))
				loss_R_t02_2 += F.l1_loss(generate_feature_4[i].unsqueeze(0), data_R_gene)
				pred = torch.softmax(model_C(data_R_real), dim=1).argmax(dim=1, keepdim=True)
				if int(grad_4[i].detach().cpu().numpy()) == 6:
					loss_C_data2 += F.cross_entropy(model_C(data_R_real), target_ppa[i])
				if pred.detach().cpu().numpy() == target_ppa[i].squeeze().detach().cpu().numpy():
					acc_num += 1
			loss_C_data2 /= R_num
			loss_R_t02_1 /= R_num
			...
			loss_R_t03_2 = 0
			loss_C_data3 = 0
			for i in range(data_3.size(0)):
				R_num += 1
				num += 1
				deltaT = int(grad_4[i].detach().cpu().numpy()) - int(grad_3[i].detach().cpu().numpy())
				data_R_real = data_3[i].unsqueeze(0)
				data_R_gene = generate_feature_3[i].unsqueeze(0)
				for ii in range(deltaT):
					data_R_real = model_R(data_R_real)
					data_R_gene = model_R(data_R_gene)
				loss_R_t03_1 += F.l1_loss(data_R_real, data_4[i].unsqueeze(0))
				loss_R_t03_2 += F.l1_loss(generate_feature_4[i].unsqueeze(0), data_R_gene)
				pred = torch.softmax(model_C(data_R_real), dim=1).argmax(dim=1, keepdim=True)
				if int(grad_4[i].detach().cpu().numpy()) == 6:
					loss_C_data3 += F.cross_entropy(model_C(data_R_real), target_ppa[i])
				if pred.detach().cpu().numpy() == target_ppa[i].squeeze().detach().cpu().numpy():
					acc_num += 1
			loss_C_data3 /= R_num
			loss_R_t03_1 /= R_num
			...
			lossC_num = 0
			for i in range(data_1.size(0)):
				if not int(grad_4[i].detach().cpu().numpy()) == 6:
					pred_t1[i] == 0
					target_ppa[i] == 0
					pred_Gt1[i] == 0
					y_hat4[i] == 0
			...
			lossC_data = loss_C_data1 + loss_C_data2 + loss_C_data3
			loss_R_1 = loss_R_t01_1 + loss_R_t02_1 + loss_R_t03_1
			loss_R_2 = loss_R_t01_2 + loss_R_t02_2 + loss_R_t03_2
			loss_G = fake_loss_1 + fake_loss_2 + fake_loss_3 + fake_loss_4
			...
			train_loss_D.update(loss_D.item(), 4 * data_1.size(0))
			train_loss_C.update(loss_C.item(), 3 * data_1.size(0))
			train_loss_R.update(loss_C.item(), 3 * data_1.size(0))
...
def main():
	...
	if args.G_net_type == 'G_net':
		G_net1 = Generator_paper1order3().cuda()
	elif args.G_net_type == 'halfG_net':
		G_net1 = Generator_paper1order3_half().cuda()
	...
	try:
		for epoch in range(1, args.epochs + 1):
			...
			train_results = train_order3(args,
			                             model_C,
			                             G_net1,
			                             D_net,
			                             model_R,
