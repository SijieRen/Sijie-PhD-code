...
def train(args,
    ...
    for batch_idx, (data_1, data_2, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(train_loader):
        ...
        if batch_idx % 5 < args.D_epoch:
            ...
            z2_attr_2 = torch.cat((z2, attr_2, grad_2.float().view(data_2.size(0), -1, 1, 1)), 1)
            generate_feature_2 = G_net(z2_attr_2, featuremap_2)
            
            real_loss_1 = D_net(featuremap_2, z2_attr_2).mean(0).view(1)
            fake_loss_1 = D_net(generate_feature_1, z2_attr_2).mean(0).view(1)
            ...
            pred_gen_2 = model_2_generate(generate_feature_2)
            ...
            for i in range(data_1.size(0)):
                ...
                if int(grad_1[i].detach().cpu().numpy()) == 1 and int(grad_2[i].detach().cpu().numpy()) == 6:
                    ...
                    reg_mono = torch.add(reg_mono, torch.max(P_current_1_i[i][1] - P_current_1_j[i][1] + args.delta1,
                                                             torch.FloatTensor([0.0]).cuda()))
                    loss_gen = torch.add(loss_gen, F.cross_entropy(pred_gen_1[i].unsqueeze(0), target_ppa[i]) +
                                         F.cross_entropy(pred_gen_2[i].unsqueeze(0), target_ppa[i]))
                    if args.train_ori_data:
                        if target_ppa[i] == 0:
                            gen_count += 2
                            loss_gen = torch.add(loss_gen, F.cross_entropy(pred_cur_i[i].unsqueeze(0), target_ppa[i]))
        ...
        if batch_idx % 5 >= args.D_epoch:
            ...
            z2 = torch.randn(data_1.size(0), 100, 1, 1).cuda()
            attr_2 = feature_2.view(data_2.size(0), -1, 1, 1)
            z2_attr_2 = torch.cat((z2, attr_2, grad_2.float().view(data_2.size(0), -1, 1, 1)),
                                  1)  # cat the extra feature random z and specific extra labels
            generate_feature_2 = G_net(z2_attr_2, featuremap_2)
            
            loss_G = D_net(generate_feature_1, z2_attr_2).mean(0).view(1)
