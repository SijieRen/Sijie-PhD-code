...
import copy
import datetime
from utils_dataloder import DataLoaderX, ppa_dataloader_order3, get_all_dataloader_order3
from utils_save_results_baseline import save_results_baseline_order3
...
def train_baseline(args,
    ...
    for batch_idx, (data_1, data_2, data_3, data_4, ppa_t1, ppa_t2, ppa_t3, target_ppa, feature_1, feature_2, feature_3, feature_4,
        ...
        if args.model == 'MM_F':
            ...
            output_generate_2 = model_1(data_2)
            output_generate_3 = model_1(data_3)

        if args.pred_future:
            loss_M_generate = F.cross_entropy(
                (output_generate_1 + output_generate_2 + output_generate_3)/3, target_ppa)
        ...
        res = F.softmax(output_generate_1, dim=1)[:, 1] - F.softmax(output_generate_2, dim=1)[:, 1] + args.delta1 + \
            F.softmax(output_generate_2, dim=1)[
            :, 1] - F.softmax(output_generate_3, dim=1)[:, 1] + args.delta1
        res[res < 0] = 0
...
def evaluate_baseline(args,
    ...
    ppa_3 = np.zeros((len(test_loader.dataset),))
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data_1, data_2, data_3, data_4, ppa_t1, ppa_t2, ppa_t3, target_ppa, feature_1, feature_2, feature_3, feature_4,
            ...
            data_1, data_2, data_3, data_4, ppa_t1, ppa_t2, ppa_t3, target_ppa, feature_1, feature_2, feature_3, feature_4, \
                grad_1, grad_2, grad_3, grad_4 = \
                data_1.cuda(), data_2.cuda(), data_3.cuda(), data_4.cuda(), ppa_t1.cuda(), ppa_t2.cuda(), ppa_t3.cuda(), \
                target_ppa.cuda(), feature_1.cuda(), \
                feature_2.cuda(), feature_3.cuda(), feature_4.cuda(), grad_1.cuda(), grad_2.cuda(), \
                grad_3.cuda(), grad_4.cuda()
            ...
            if args.model == 'MM_F':
                ...
                output_generate_1 = model_1(data_1) / 3 + \
                    model_1(data_2) / 3 + \
                    model_1(data_3) / 3
            ...
            #     if ppa_t1[i].detach().detach().cpu().numpy()[0] == 1 or ppa_t2[i].detach().detach().cpu().numpy()[0] == 1 or ppa_t3[i].detach().detach().cpu().numpy()[0] == 1:
            #         pred_generate[i] = 1
            ...
            ppa_3[batch_begin:batch_begin +
                  data_1.size(0)] = ppa_t3.cpu().numpy()
...
def main():
    ...
    train_loader, val_loader_list, test_loader_list = get_all_dataloader_order3(
        args)

    model_1 = resnet18(num_classes=args.class_num, inputchannel=3).cuda()
    # if args.model == 'MM_F':
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            args.logger.info('Ours BS-order3')
