...
class Encoder(nn.Module):
    def __init__(self, in_num_ch=1, img_size=(64,64,64), inter_num_ch=16, kernel_size=3, conv_act='leaky_relu', num_conv=2, dropout=False):
        ...
        self.conv4 = nn.Sequential(
                        ...
                        nn.Conv2d(inter_num_ch, inter_num_ch, kernel_size=kernel_size, padding=1))
...
class Decoder(nn.Module):
    def __init__(self, out_num_ch=1, img_size=(64,64), inter_num_ch=16, kernel_size=3, conv_act='leaky_relu', num_conv=2, shortcut=False):
        ...
        if self.shortcut:
            ...
            self.conv0 = nn.Conv2d(inter_num_ch, out_num_ch, kernel_size=3, padding=1)
        else:
            self.conv4 = DecoderBlock(inter_num_ch, 4*inter_num_ch, kernel_size=kernel_size, conv_act=conv_act, dropout=0, num_conv=num_conv)
    ...
    def forward(self, x, feat_list=[]):
        ...
        x_reshaped = x.view(x.shape[0], 16, 4, 4)
...
class SOMPairVisit(SOM):
    def __init__(self, config):
        ...
        self.encoder = Encoder(in_num_ch=3, inter_num_ch=16, num_conv=2)
        self.decoder = Decoder(out_num_ch=3, inter_num_ch=16, num_conv=2, shortcut=False)
        self.embeddings = nn.Parameter(torch.fmod(torch.randn(self.embedding_size[0],self.embedding_size[1],self.latent_size,requires_grad=True),2) * 0.05, requires_grad=True)
...
class LSOR_cls(nn.Module):
    def __init__(self, latent_size=256, inter_num_ch=128, inter_num_ch2=128):
        super(LSOR_cls, self).__init__()
        self.fc = nn.Sequential(
                    
                    nn.Linear(latent_size, inter_num_ch),
                    nn.BatchNorm1d(inter_num_ch),
                    nn.ReLU(),

                    # nn.Dropout(0.2),
                    nn.Linear(inter_num_ch, inter_num_ch2),
                    nn.BatchNorm1d(inter_num_ch2),

                    nn.ReLU(),
                    nn.Linear(inter_num_ch2, 2))
