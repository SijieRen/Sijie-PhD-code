...
# define model
if config['model_name'] in ['SOMPairVisit']:
    model_lror = SOMPairVisit(config=config).to(config['device'])
else:
    raise ValueError('Not support other models yet!')

optimizer_lsor = torch.optim.Adam(model.parameters(), lr=config['lr'], betas=(0.9, 0.999), weight_decay=1e-5, amsgrad=True)
scheduler_lsor = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=10, min_lr=1e-5)
...
def train_baseline(args,
    ...
    for batch_idx, (data_1, data_2, data_3, ppa_t1, ppa_t2, target_ppa, feature_1, feature_2, feature_3,
        ...
        if args.pred_future:
            loss_M_generate = F.cross_entropy((output_generate_1 + output_generate_2)/2, target_ppa)
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data_1, data_2, data_3, ppa_t1, ppa_t2, target_ppa, feature_1, feature_2, feature_3,
            ...
            if args.model == 'MM_F':
                ...
                output_generate_1 = model_2_generate(featuremap_1) / 2 + model_2_generate(featuremap_2) / 2
