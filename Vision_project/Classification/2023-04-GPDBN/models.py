...
class ResNet_last_e(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, dropout=0, dp=0.5):
        super(ResNet_last_e, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.layer3 = self._make_layer(block, 1024, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, 1536, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))

        if dropout == 0:
            self.fc = nn.Linear(1536 * block.expansion, 2048)
        elif dropout == 1:
            self.fc = nn.Sequential(
                nn.Linear(1536 * block.expansion, 2048),
                nn.Dropout(dp),
                nn.Linear(256, num_classes),
            )
        self.fc_e1 = nn.Linear(2048, 4096)
        self.fc_e2 = nn.Linear(4096, 4096)
        self.fc_e22 = nn.Linear(4096, 4096)
        self.fc_e3 = nn.Linear(4096, num_classes)
...
class ResNet_last_e_extractor(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, dropout=0, dp=0.5):
        super(ResNet_last_e_extractor, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.fc2_e1 = nn.Linear(2048, 4096)
        self.fc2_e2 = nn.Linear(4096, 4096)
        self.fc2_e22 = nn.Linear(4096, 4096)
        self.fc2_e3 = nn.Linear(4096, num_classes)

        self.fc1_age = nn.Linear(2048, 2048)
        self.fc2_age = nn.Linear(2048, 2048)
        self.fc3_age = nn.Linear(2048, 2048)
        self.bn_age = nn.BatchNorm1d(num_features=2048)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(
                    m.weight, mode='fan_out', nonlinearity='relu')
...
class ResNet_last_attr_e(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, feature_dim=6, dropout=0, dp=0.5):
        super(ResNet_last_attr_e, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.layer4 = self._make_layer(block, 1536, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # encoder zuihou yiceng
        if dropout == 0:
            self.fc_0 = nn.Linear(1536 * block.expansion, 32)
        elif dropout == 1:
            self.fc_0 = nn.Sequential(
                nn.Linear(1536 * block.expansion, 2048),
                nn.Dropout(dp),
        ...
        self.fc_e1 = nn.Linear(124, 4096)
        self.fc_e2 = nn.Linear(4096, 4096)
        self.fc_e22 = nn.Linear(4096, 4096)
        self.fc_e3 = nn.Linear(4096, num_classes)

        self.fc_1024_to_20_1 = nn.Linear(1024, 20)
        self.fc_1024_to_20_2 = nn.Linear(1024, 20)
        self.fc_1024_to_20_3 = nn.Linear(1024, 20)

        # encoder zuihou yiceng
        if dropout == 0:
            self.fc_01 = nn.Linear(256, 32)
        elif dropout == 1:
            self.fc_01 = nn.Sequential(
                nn.Linear(256, 512),
                nn.Dropout(dp),
    ...
    def forward(self, x, x1):
        ...
        if x1 is None:
            pass
        else:  # 构建一个简单的MLP # sijie GPDBN modification
            x1 = self.fc1(x1)
            x1 = self.bn1(x1)
            x1 = self.relu(x1)
            x1 = self.fc2(x1)
...
def RN18_last_e(**kwargs):
    return ResNet_last_e(BasicBlock, [2, 2, 2, 2], **kwargs)


def RN18_last_e_extractor(**kwargs):
    return ResNet_last_e_extractor(BasicBlock, [2, 2, 2, 2], **kwargs)


def RN18_last_attr_e(**kwargs):
    return ResNet_last_attr_e(BasicBlock, [2, 2, 2, 2], **kwargs)
        ...
        if self.task_num == 1:
            ...
            if type(idx2) == int:
                ...
                for s in range(1, x.size(0)):
                    pred = torch.cat(
                        [pred, self.fc[idx1[s]](x[s].unsqueeze(0))], dim=0)
                if feature:
                    return pred, x
                else:
                    return pred
                ...
                for s in range(1, x.size(0)):
                    ...
                    pred = torch.cat(
                        [pred, self.fc[true_idx](x[s].unsqueeze(0))], dim=0)
                if feature:
                    return pred, x
                else:
                    return pred
...
class Generator(nn.Module):
    def __init__(self, feature_num=6, is_ESPCN=0, mid_channel=1024):
        ...
        self.conv_blocks1 = nn.Sequential(
            ...
            nn.ConvTranspose2d(in_channels=100 + feature_num,
                               out_channels=2048, kernel_size=4, stride=1, padding=0),
            # up4
            ...
            nn.ConvTranspose2d(
                in_channels=2048, out_channels=1024, kernel_size=4, stride=2, padding=1),
            ...
            # 8*8 - 16*16
            nn.ConvTranspose2d(
                in_channels=1024, out_channels=1024, kernel_size=4, stride=2, padding=1),
            # up2:4-2-1 up4:4-4-0 8-4-2
            nn.BatchNorm2d(num_features=1024),
            nn.ReLU(inplace=True),
        ...
        self.conv_blocks2 = nn.Sequential(
            # 32*32 - 64*64
            nn.ConvTranspose2d(
                in_channels=1024, out_channels=256, kernel_size=4, stride=2, padding=1),
            # up2:4-2-1 up4:4-4-0 8-4-2
            nn.BatchNorm2d(num_features=256),
            nn.ReLU(inplace=True),
            # 64*64 - 64*64
...
class Generator_seq(nn.Module):
    def __init__(self, feature_num=6, final_tanh=0, in_channel=128):
        ...
        self.conv_blocks_feature = nn.Sequential(
            # 128*64*64 - 5256*32*32
            nn.Conv2d(in_channels=self.in_channel, out_channels=256,
                      kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(num_features=256),
            nn.ReLU(inplace=True),
...
class Generator_LSTM_1(nn.Module):
    def __init__(self, feature_num=6, final_tanh=0, in_channel=128, RNN_hidden=[512]):
        ...
        self.conv_blocks2 = nn.Sequential(
            # 32*32 - 64*64
            nn.ConvTranspose2d(
                in_channels=self.RNN_hidden[-1], out_channels=256, kernel_size=4, stride=2, padding=1),
            # up2:4-2-1 up4:4-4-0 8-4-2
            nn.BatchNorm2d(num_features=256),
            nn.ReLU(inplace=True),
            # 64*64 - 64*64
...
class Generator_LSTM_1_ind(nn.Module):
    ...
    def forward(self, feature1, z1, grad_1, grad_2, return_all=0):  # 这里G进来的是batch中的一个object
        mid_feature_z_1 = self.conv_blocks1(z1)
        mid_feature_img_1 = self.conv_blocks_feature(feature1)
        x1 = torch.cat((mid_feature_z_1, mid_feature_img_1), 1)
        hidden = None
        ...
        for idx in range(6):
            if len(self.RNN_hidden) == 1:
                if idx >= int(grad_1) and idx <= int(grad_2):  # 这里进行时序不同次数的LSTM
                    hidden = self.LSTM_cell[0](x1, hidden)
                else:
                    hidden = None
                if idx == int(grad_2) - 1:
                    all_hidden.append(hidden)  # 只有最后一个time point生成数据
            ...
            if len(self.RNN_hidden) == 2:
                ...
                if idx == int(grad_2) - 1:
                    all_hidden.append(hidden)
                    delta_t_list.append(idx - 1)
        if return_all == 1:  # return all = 0
            featuremap = []
            i = 0  # ind 版本增加了 i参数
            for hidden in all_hidden:
                if hidden is not None:
                    featuremap.append(self.conv_blocks2[delta_t_list[i]](
                        hidden[0]))  # 这里不同时序信息的LSTM结果需要过不同的conv blosck2
                    i = i + 1
                else:
                    featuremap.append(None)
            return featuremap
    ...
    def forward(self, feature1, z1, grad_1, feature2, z2, grad_2, grad_3, return_all=0):
        ...
        if return_all == 1:
            ...
            for hidden in all_hidden:
                if hidden is not None:
                    featuremap.append(
                        self.conv_blocks2[delta_t_list[ii]](hidden[0]))
                    ii = ii + 1
                else:
                    featuremap.append(None)
            return featuremap
    ...
    def __init__(self, channels):
        ...
        self.main_module = nn.Sequential(
            ...
            nn.Conv2d(in_channels=channels, out_channels=256,
                      kernel_size=4, stride=2, padding=1),  # modify
            nn.BatchNorm2d(num_features=256),
...
class ESPCN(nn.Module):
    def __init__(self, num_channels=128, mid_channel=2048, scale_factor=3):
        super(ESPCN, self).__init__()
        self.scale_factor = scale_factor
        self.first_part = nn.Sequential(
            nn.Conv2d(num_channels, mid_channel,
                      kernel_size=5, padding=5 // 2),
            nn.ReLU(),
            nn.Conv2d(mid_channel, mid_channel // 2,
                      kernel_size=3, padding=3 // 2),
            nn.ReLU(),
        )
        self.last_part = nn.Sequential(
            nn.Conv2d(mid_channel // 2, num_channels *
                      (pow(scale_factor, 2)), kernel_size=3, padding=3 // 2),
