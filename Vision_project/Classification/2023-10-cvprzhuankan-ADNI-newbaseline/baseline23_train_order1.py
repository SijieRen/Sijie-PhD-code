...
from data.dataloader import ExtractorData, Order1Data, Order2Data
from model import *
import numpy as np
...
def train(opt):
    # Datasets
    if opt.data_order == 1:
        dataset_train = Order1Data(data_index='./data/dataset_order%s_ouou.xls' % '1',
                                   data_root=opt.data_root, mode='train')
        dataset_test = Order1Data(data_index='./data/dataset_order%s_ouou.xls' % '1',
                                  data_root=opt.data_root, mode='test')
    elif opt.data_order == 2:
        dataset_train = Order2Data(data_index='./data/dataset_order%s_ouou.xls' % '2',
                                   data_root=opt.data_root, mode='train')
        dataset_test = Order2Data(data_index='./data/dataset_order%s_ouou.xls' % '2',
                                  data_root=opt.data_root, mode='test')
    else:
    ...
    for epoch in range(int(opt.epoch)):  # sijie modification
        model_baseline.train()

        total_step = len(dataloader_train)
        for step, (data_3D, data_T, label, time_t, time_T) in enumerate(dataloader_train):
            # data_3D, label = data_3D.cuda().float(), label.cuda().float()
            #data_t, data_T, label, time_t, time_T
            data_3D, data_T, label, time_t, time_T = data_3D.cuda().float(), data_T.cuda(
            ).float(), label.cuda().long(), time_t.cuda().float(), time_T.cuda().float()
            data_3D = F.interpolate(data_3D, scale_factor=opt.data_scale)
            delta_t = (time_T - time_t)/12

            if opt.baseline == 'FFC':
                pred = model_baseline(data_3D)
            elif opt.baseline == 'GPDB':
                pred = model_baseline(data_3D, delta_t)
            # print(label.argmax(dim=1, keepdim=False),
            #       label.argmax(dim=1, keepdim=False).size())
            loss = F.cross_entropy(pred, label.argmax(dim=1, keepdim=False))
        ...
        pred_all = []
        label_all = []
        for i, (data_3D, data_T, label, time_t, time_T) in tqdm(enumerate(dataloader_test)):
