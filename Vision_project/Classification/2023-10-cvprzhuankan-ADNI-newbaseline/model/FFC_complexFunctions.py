...
import torch
from torch.nn.functional import relu, max_pool2d, dropout, dropout2d, adaptive_avg_pool2d, max_pool3d, dropout3d, adaptive_avg_pool3d
...
def complex_max_pool3d(input_r,input_i,kernel_size = 2, stride=2, padding=0,
                                dilation=1, ceil_mode=False, return_indices=False):

    return max_pool3d(input_r, kernel_size, stride, padding, dilation,
                      ceil_mode, return_indices), \
           max_pool3d(input_i, kernel_size, stride, padding, dilation,
                      ceil_mode, return_indices)
...
def complex_AdaptiveAvgPool3d(input_r,input_i,output_size=[1,1]):

    return adaptive_avg_pool3d(input_r, output_size), \
           adaptive_avg_pool3d(input_i, output_size)
...
def complex_dropout3d(input_r,input_i, p=0.5, training=True, inplace=False):
    return dropout3d(input_r, p, training, inplace), \
           dropout3d(input_i, p, training, inplace)
