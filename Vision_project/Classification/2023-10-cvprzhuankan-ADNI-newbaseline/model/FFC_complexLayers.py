...
from torch.nn import Conv2d, Linear, BatchNorm1d, BatchNorm2d, BatchNorm3d
from torch.nn import ConvTranspose2d, ConvTranspose3d, Conv3d
from FFC_complexFunctions import complex_relu, complex_max_pool2d
from FFC_complexFunctions import complex_dropout, complex_dropout2d, complex_AdaptiveAvgPool2d
from FFC_complexFunctions import complex_max_pool3d, complex_dropout3d, complex_AdaptiveAvgPool3d
...
class Complex_AdaptiveAvgPool3d(Module):
    def __init__(self, output_size=[1,1]):
        super(Complex_AdaptiveAvgPool3d,self).__init__()
        self.output_size = output_size
   
    def forward(self,input_r,input_i):
        return complex_AdaptiveAvgPool3d(input_r,input_i,output_size=self.output_size)
...
class ComplexDropout3d(Module):
    ...
    def forward(self,input_r,input_i):
        return complex_dropout3d(input_r,input_i,self.p,self.inplace)
...
class ComplexMaxPool3d(Module):
    ...
    def forward(self,input_r,input_i):
        return complex_max_pool3d(input_r,input_i,kernel_size = self.kernel_size,
                                stride = self.stride, padding = self.padding,
                                dilation = self.dilation, ceil_mode = self.ceil_mode,
                                return_indices = self.return_indices)
...
class ComplexConv3d(Module):      # 感觉这样直接算可能确实有问题哦，相当于直接用它来做实数间的运算了，看不出这个梯度之间的联系了，本身复数还算是个整体
