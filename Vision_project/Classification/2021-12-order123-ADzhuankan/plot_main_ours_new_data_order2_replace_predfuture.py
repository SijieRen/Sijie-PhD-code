...
def train(args,
    ...
    for batch_idx, (data_1, data_2, data_3, ppa_t1, ppa_t2, target_ppa, feature_1, feature_2, feature_3,
        ...
        if batch_idx % args.GD_epoch < args.D_epoch:
            ...
            # Sijie 尝试去掉pred future ，并增加对GAN结果的多重cla loss, 这样model1 暂时不再使用
            P_pred_future = [
            ...
            # 包含current 和 progression， 思捷修改ourmethod curr部分
            P_pred = [torch.softmax(model_2_list[0](generate_feature_2_by_1, None), dim=1)/2
                                    + torch.softmax(model_2_list[0](featuremap_2, torch.cat(
                [feature_2, grad_2.float().view(data_2.size(0), -1)], 1)), dim=1)/2,
            ...
            P_pred_gen = [ # f_curr
                ...
                model_2_list[0](featuremap_2, torch.cat(# 是否增加真实T数据训练
            ...
            for i in range(data_1.size(0)):
                ...
                loss_gen = torch.add(loss_gen,
                                        F.nll_loss(torch.log(eps + P_pred[0][i].unsqueeze(0)), ppa_t2[i])+ \
                ...
                loss_cur = torch.add(loss_cur,
                                        ...
                                        F.cross_entropy(P_pred_gen[3][i].unsqueeze(0), ppa_t2[i])
                                        )
                ...
                reg_mono = torch.add(reg_mono,
                                     ...
                                     torch.max(F.softmax(P_pred_gen[1][i].unsqueeze(0), dim=1)[0, 1] -# P_pred_3_by_12由P_pred_gen[1]计算而来，增加单调限制
                                               P_pred_3_by_12[i][1] + args.delta1,
                                               torch.FloatTensor([0.0]).cuda()) + \

                                     torch.max(F.softmax(P_pred_future[0][i].unsqueeze(0), dim=1)[0, 1] -#P_pred_future增加单调限制 1-2 1-3 2-3
                                               F.softmax(P_pred_future[1][i].unsqueeze(0), dim=1)[0, 1] + args.delta1,# 1->3 > 1->2
                                               torch.FloatTensor([0.0]).cuda()) + \
...
def evaluate(args,
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data_1, data_2, data_3,  ppa_t1, ppa_t2,target_ppa, feature_1, feature_2, feature_3,
            ...
            res_feature_2_1 = generate_feature_3_by_12 - featuremap_1
            ...
            real_order2_feature = (featuremap_3 - featuremap_2) - (featuremap_2 - featuremap_1)
                ...
                for i in range(data_1.size(0)):
                    ...
                    for plotppa in range(2):
                        ...
                        if target_ppa[i] == plotppa:
                            ...
                            ### order2 feature
                            unet_feature1_1 = real_order2_feature[i, :, :, :]  # add the unnomarlization
                            ...
                            save_dir0 = (save_dir + 'order2_feature_real/')
                            ...
                            unet_feature1_1 = real_order2_feature[i, :, :, :].mul(-1)  # add the unnomarlization
                            unet_feature1_1 = torch.unsqueeze(unet_feature1_1, 0)
                            if args.plot_sigmoid:
                                unet_feature1_1 = torch.sigmoid(unet_feature1_1)
                            unet_feature1_1 = unet_feature1_1.cpu().detach().numpy()
                            save_dir0 = (save_dir + 'order2_feature_real_minus/')
                            draw_features(16, 8, unet_feature1_1, save_dir0, image_sequence, test_loader,
    ...
    #args.logger.info("target: ", target)
    #args.logger.info("pred: ", pred_final)
...
def main():
    ...
    for ss in range(len(model_2_list)):
        load_pytorch_model22(model_2_list[ss],
                             r'../results/Ecur_2201-Ours-order2/2024-06-29-16-11-12_Mixed_0.000200_80.000000_size_256_ep_120_0_R_Maculae/best_val_auc_model_2_list.pt',
                             list=ss)
    for ii in range(len(G_net_list)):
        load_pytorch_model22(G_net_list[ii],
                             r'../results/Ecur_2201-Ours-order2/2024-06-29-16-11-12_Mixed_0.000200_80.000000_size_256_ep_120_0_R_Maculae/best_val_auc_G_net_list.pt',
                             list=ii)
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            args.logger.info('Ours method-order2')
        ...
        args.logger.info('Ours method-order2_visualization')
        args.logger.info('-' * 50)
        args.logger.info('-' * 50)
        args.logger.info('-' * 50)
