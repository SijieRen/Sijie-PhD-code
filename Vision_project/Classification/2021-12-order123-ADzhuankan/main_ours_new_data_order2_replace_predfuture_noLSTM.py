...
from models_order_2_3 import Generator_seq_ind as Generator
from utils import *
        ...
        if batch_idx % args.GD_epoch < args.D_epoch:
            ...
            for idx in range(data_1.size(0)):
                all_generated_feature = [G_net_list[0](#feature1=featuremap_1[idx].unsqueeze(0),
                                                      feature=featuremap_1[idx].unsqueeze(0),
                                                      z=z_attr_1[idx].unsqueeze(0),
                                                      #z2=z_attr_2[idx].unsqueeze(0),
                                                      # grad_1=grad_1[idx].detach().cpu().numpy() - 1,
                                                      grad_1=grad_1[idx].detach().cpu().numpy() - 1,
                                                      grad_2=grad_2[idx].detach().cpu().numpy() - 1)]
                all_generated_feature.append(
                    G_net_list[0](  # feature1=featuremap_1[idx].unsqueeze(0),
                        feature=featuremap_2[idx].unsqueeze(0),
                        z=z_attr_2[idx].unsqueeze(0),
                        # z2=z_attr_2[idx].unsqueeze(0),
                        # grad_1=grad_1[idx].detach().cpu().numpy() - 1,
                        grad_1=grad_2[idx].detach().cpu().numpy() - 1,
                        grad_2=grad_3[idx].detach().cpu().numpy() - 1)
                )
                if idx == 0:
...
def main():
    ...
    load_pytorch_model(model_1, \
                        r'../results/Ecur_211217_BS_Extractor/2021-12-21-23-35-07_Mixed_0.002000_50.000000_size_256_ep_80_0_R_Maculae/best_val_auc_model_1.pt')
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            args.logger.info('Ours method-order2_noLSTM')
