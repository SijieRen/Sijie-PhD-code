...
def evaluate(args,
    ...
    pred_result_res = np.zeros((len(test_loader.dataset), args.class_num))
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data_1, data_2, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(test_loader):
            ...
            pred_result_res[batch_begin:batch_begin + data_1.size(0), :] = P_residue_1_i.detach().cpu().numpy()
    ...
    AUC_res = sklearn.metrics.roc_auc_score(target, pred_result_res[:, 1])
    acc_res = sklearn.metrics.accuracy_score(target, np.argmax(pred_result_res, axis=1))
    ...
    results = {
        ...
        'pred_result_res': pred_result_res,
        'AUC_res': AUC_res,
        'acc_res': acc_res,
