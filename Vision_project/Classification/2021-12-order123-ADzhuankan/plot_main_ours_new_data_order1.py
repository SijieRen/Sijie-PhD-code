    ...
    for batch_idx, (data_1, data_2, ppa_t1, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(train_loader):
        ...
        if batch_idx % 5 < args.D_epoch:
            ...
            for i in range(data_1.size(0)):# batch内循环
                ...
                # args.logger.info(pred_gen_1[i].unsqueeze(0).size())
                # args.logger.info(target_ppa[i].size())
                # args.logger.info(pred_gen_1[i].unsqueeze(0))
                # args.logger.info(target_ppa[i])
                ...
                for i in range(data_1.size(0)):
                    ...
                    for plotppa in range(2):
                        ...
                        if target_ppa[i] == plotppa:
                            ...
                            featuremap_2_1 = featuremap_1[i, :, :, :] - featuremap_2[i, :, :, :]
                            featuremap_2_1 = torch.unsqueeze(featuremap_2_1, 0)
                            if args.plot_sigmoid:
                                featuremap_2_1 = torch.sigmoid(featuremap_2_1)
                            featuremap_2_1 = featuremap_2_1.cpu().detach().numpy()
...
def main():
    ...
    load_pytorch_model(model_2_generate,
                       ...
                       r'../results/Ecur_2201-Ours-order1/2024-06-24-08-09-26_Mixed_0.000200_60.000000_size_256_ep_120_0_R_Maculae/best_val_auc_model_2_generate.pt')
    load_pytorch_model(model_2_res,#os.path.join(args.load_visual ,'best_val_auc_model_2_res.pt'))
                       r'../results/Ecur_2201-Ours-order1/2024-06-24-08-09-26_Mixed_0.000200_60.000000_size_256_ep_120_0_R_Maculae/best_val_auc_model_2_res.pt')
    load_pytorch_model(G_net,#os.path.join(args.load_visual ,'best_val_auc_G_net.pt'))
                       r'../results/Ecur_2201-Ours-order1/2024-06-24-08-09-26_Mixed_0.000200_60.000000_size_256_ep_120_0_R_Maculae/best_val_auc_G_net.pt')
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            args.logger.info('Ours method-order1')
        ...
        save_featur_path = '../Feature_map/%s-Label%s/' % (args.sequence, "0-1")
        args.logger.info('save_featuremap_path: %s' % save_featur_path)
        args.logger.info('Ours method-order1_visualization')
        args.logger.info('-' * 50)
        args.logger.info('-' * 50)
        args.logger.info('-' * 50)
