        ...
        if batch_idx % args.GD_epoch < args.D_epoch:
            ...
            P_pred = [torch.softmax(model_2_list[1](featuremap_1, feature_1, #包含current 和 progression
                                    ...
                                    model_2_list[1](featuremap_2, feature_2,
                                                    idx1=(grad_3 - grad_2 - 1).view(data_2.size(0), ).detach().cpu().numpy()) + \
                                    model_2_list[0](featuremap_3, torch.cat([feature_3,grad_3.float().view(data_2.size(0), -1)], 1)), dim=1),
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data_1, data_2, data_3, data_4, ppa_t1, ppa_t2, ppa_t3, target_ppa, feature_1, feature_2, feature_3, feature_4,
            ...
            pred_y3_cur[batch_begin:batch_begin + data_1.size(0), :] = \
                P_pred_4_by_123.detach().cpu().numpy() / 2 + P_pred_gen.detach().cpu().numpy() / 2
