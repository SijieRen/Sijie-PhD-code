...
def train(args,
    ...
    for batch_idx, (data_1, data_2, data_3, data_4, ppa_t1, ppa_t2, ppa_t3, target_ppa, feature_1, feature_2, feature_3, feature_4,
        ...
        if batch_idx % 5 < args.D_epoch:
            ...
            P_pred_3_by_12 = P_pred[-2][:, 1] + P_pred[-2][:, 0] * (P_pred[-1][:, 1])# 和botong确认，实际上这里预测y_K,似乎没有严格按照高阶推理公式（包括tets中的最终结果）
            ...
            for i in range(data_1.size(0)):
                ...
                if target_ppa[i].detach().cpu().numpy()[0] == 0:#这里需要修改，==0判断对应原来没有中间数据
                    loss_cls_count += 1
