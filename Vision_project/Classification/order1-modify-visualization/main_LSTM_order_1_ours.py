...
class Clas_ppa_train(data.Dataset):
    def __init__(self,
        ...
        elif filename == 'minmax':
            workbook1 = xlrd.open_workbook(
                r"../ppa-classi-dataset-onuse/ppa_06-19-order1-minmax-8.xls")
        elif filename == 'std':
            workbook1 = xlrd.open_workbook(
                r"../ppa-classi-dataset-onuse/ppa_06-29-order1-std-8.xls")
        elif filename == '1-6-full':
            print('load minmax model')
            workbook1 = xlrd.open_workbook(
                r"../ppa-classi-dataset-onuse/ppa_07-02-order1-std-1-6all-8.xls")
        elif filename == 'plot_test18':
            print('load data std all pair')
            workbook1 = xlrd.open_workbook(
                r"../ppa-classi-dataset-onuse/ppa_07-02-order1-std-11-06-test24.xls")
        elif filename == 'plot_test---':
            print('load data std all pair')
            workbook1 = xlrd.open_workbook(
                r"../ppa-classi-dataset-onuse/01-25-test-visual.xls")
        elif filename == 'plot_test---chakan':
            print('load data std all pair')
            workbook1 = xlrd.open_workbook(
                r"../ppa-classi-dataset-onuse/01-25-test-visual.xls")
...
def train(args,
    ...
    for batch_idx, (data_1, data_2, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(train_loader):
        ...
        if batch_idx % 5 < args.D_epoch:
            ...
            P_current_1_j = torch.softmax(model_2_generate(featuremap_2), dim=1)
            ...
            pred_gen_1 = model_2_generate(generate_feature_1)
            ...
            loss = loss_res + args.alpha * reg_mono + args.gamma * loss_gen
        ...
        if batch_idx % 5 >= args.D_epoch:
            ...
            loss_temp = loss_res + args.alpha * reg_mono + args.gamma * loss_gen
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data_1, data_2, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(test_loader):
            ...
            for ii in range(len(name)):
                if '20202261' in name[ii]:
                    if ii <= 19:
                        print(name[ii])
                        print(pred_label_average_all[ii])
                        jj = ii
                        print('as for real 1111111111 :')
                        print(sklearn.metrics.pairwise.cosine_similarity(feature_1[jj].reshape(-1, 1).detach().cpu().numpy(),
                                                                         feature_1[jj].reshape(-1, 1).detach().cpu().numpy()))
                        print(feature_1[jj].reshape(-1, 1).size())
                        print('as for real 2222222222 :')
                        print(sklearn.metrics.pairwise.cosine_similarity(feature_2[jj].reshape(-1, 1).detach().cpu().numpy(),
                                                                         feature_2[jj].reshape(-1, 1).detach().cpu().numpy()))
                    else:
                        print(name[ii])
                        print(pred_label_average_all[ii])
                        jj = ii - 20
                        print('as for real 1111111111 :')
                        #print(sklearn.metrics.pairwise.cosine_similarity(featuremap_1[jj].reshape(128,4096).detach().cpu().numpy(),
                                                                         #featuremap_1[jj].reshape(128,4096).detach().cpu().numpy()))
                        a = sklearn.metrics.pairwise.cosine_similarity(torch.sigmoid(featuremap_1[jj]).reshape(128,4096).detach().cpu().numpy(),
                                                                         torch.sigmoid(featuremap_1[jj]).reshape(128,4096).detach().cpu().numpy())
                        #print(a)
                        print('cosine similarity :', (a.sum() - 128)/(128*127))
                        a_ = sklearn.metrics.pairwise.pairwise_distances(torch.sigmoid(featuremap_1[jj]).reshape(128,4096).detach().cpu().numpy(),
                                                                         torch.sigmoid(featuremap_1[jj]).reshape(128,4096).detach().cpu().numpy(), metric='cosine')
                        print(a_)
                        print('pairwise distance :', (a_.sum())/(128*127))
                        #print(featuremap_1[jj].size())
                        #print(featuremap_1[jj])
                        #print(featuremap_1[jj].reshape(-1, 1).size())
                        #print(featuremap_1[jj].reshape(128,4096).size())
                        print('as for real 2222222222 :')
                        #print(sklearn.metrics.pairwise.cosine_similarity(featuremap_2[jj].reshape(128,4096).detach().cpu().numpy(),
                                                                         #featuremap_2[jj].reshape(128,4096).detach().cpu().numpy()))
                        b = sklearn.metrics.pairwise.cosine_similarity(torch.sigmoid(featuremap_2[jj]).reshape(128,4096).detach().cpu().numpy(),
                                                                         torch.sigmoid(featuremap_2[jj]).reshape(128,4096).detach().cpu().numpy())
                        #print(b)
                        print((b.sum() - 128)/(128*127))
                        
                        b_fake = np.zeros((128,4096))
                        pmax = 0.0001
                        pmin = -0.0001
                        for kk in range(128):
                            #pass
                            for ll in range(4096):
                                pmin = np.min(featuremap_2[jj].reshape(128,4096)[kk].detach().cpu().numpy())
                                pmax = np.max(featuremap_2[jj].reshape(128, 4096)[kk].detach().cpu().numpy())
                                
                            for ll in range(4096):
                                b_fake[kk][ll] = ((featuremap_2[jj].reshape(128,4096)[kk][ll].detach().cpu().numpy() - pmin)
                                                  / (pmax - pmin + 0.000001))
                                pass
                        
                        b_ = sklearn.metrics.pairwise.pairwise_distances(b_fake,
                                                                         b_fake, metric='cosine')
                        print(b_)
                        print('pairwise distance :', (b_.sum())/(128*127))
            print('-' * 20)
            print('-' * 20)
            
            #print('pred labe :', pred_label_average_all)
            ...
            if args.is_plot == 1:
                ...
                for i in range(data_1.size(0)):
                    ...
                    if target_ppa[i] == 1:
                        pass
                        '''
                        save_dir = '../Feature_map/Order1_%s-Label1/' % (args.sequence,)
                        ...
                        save_dir = '../Feature_map/Order1_%s-Label0/' % (args.sequence,)
                        ...
                        featuremap_2_1 = featuremap_2[i, :, :, :] - featuremap_1[i, :, :, :]
                        featuremap_2_1 = torch.unsqueeze(featuremap_2_1, 0)
                        if args.plot_sigmoid:
                            featuremap_2_1 = torch.sigmoid(featuremap_2_1)
                        featuremap_2_1 = featuremap_2_1.cpu().detach().numpy()
                        save_dir4 = (save_dir + 'real_minus')
                        draw_features(16, 8, featuremap_2_1, save_dir4, image_sequence, test_loader, 'real_minus',
                                      args.batch_size, args.dpi, args.is_local_normal, args)
                        ...
                        featuremap_2_1 = featuremap_2_1.cpu().detach().numpy()
                        save_dir4 = (save_dir + 'real_minus')
                        draw_features(16, 8, featuremap_2_1, save_dir4, image_sequence, test_loader, 'real_minus_minus',
                                      args.batch_size, args.dpi, args.is_local_normal, args)
...
def main():
    ...
    load_pytorch_model(model_1,
                       os.path.join(args.load_visual ,'best_val_auc_model_1.pt'))
    load_pytorch_model(model_2_generate,
                       os.path.join(args.load_visual, 'best_val_auc_model_2_generate.pt'))
                       #r'../results/Ecur_10-31-modify/order1-ours-003/2020-10-31-22-50-13_Mixed_0.000200_60.000000_size_256_ep_120_0_R_Maculae/best_val_auc_model_2_generate.pt')
    load_pytorch_model(model_2_res,os.path.join(args.load_visual ,'best_val_auc_model_2_res.pt'))
                       #r'../results/Ecur_10-31-modify/order1-ours-003/2020-10-31-22-50-13_Mixed_0.000200_60.000000_size_256_ep_120_0_R_Maculae/best_val_auc_model_2_res.pt')
    load_pytorch_model(G_net,os.path.join(args.load_visual ,'best_val_auc_G_net.pt'))
                       #r'../results/Ecur_10-31-modify/order1-ours-003/2020-10-31-22-50-13_Mixed_0.000200_60.000000_size_256_ep_120_0_R_Maculae/best_val_auc_G_net.pt')
