...
class Clas_ppa_train(data.Dataset):
    def __init__(self,
        ...
        if filename == 'std':
            print('load std model')
            workbook1 = xlrd.open_workbook(
                "../ppa-classi-dataset-onuse/ppa_06-29-order1-std-8.xls")
        elif filename == 'no':
            print('load no normalize model')
            workbook1 = xlrd.open_workbook(
...
def train_baseline(args,
    ...
    for batch_idx, (data_1, data_2, target_ppa, feature_1, feature_2, grad_1, grad_2) in enumerate(train_loader):
        ...
        if args.pred_future:
            loss_M_generate = F.cross_entropy(output_generate_1, target_ppa)
