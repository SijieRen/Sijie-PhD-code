...
def main():
    ...
    if args.G_net_type == 'G_net':
        G_net1 = Generator_paper1oreder1().cuda()
    ...
    if args.optimizer == 'Mixed':
        ...
        #optimizer_R = optim.Adam(model_R.parameters(), lr=args.lr3)
        #optimizer_ESPCN = optim.Adam([
            #{'params': model_ESPCN.first_part.parameters()},
            #{'params': model_ESPCN.last_part.parameters(), 'lr': args.lr3 * 0.1},
            #{'params': model_ESPCN.down.parameters()},
        #], lr=args.lr3)
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            train_results = train(args, model_C,  G_net1, D_net, model_R,
                                  train_loader,optimizer_C, optimizer_G1, optimizer_D, optimizer_R,
                                  epoch)
            test_results_list = []
            val_results_list = []
            for ss in range(len(val_loader_list)):
                test_results_list.append(
                    evaluate(args, model_R, model_C,G_net1,
                             test_loader_list[ss], epoch, 5-ss))
                val_results_list.append(
                    evaluate(args, model_R, model_C,G_net1,
                             val_loader_list[ss], epoch, 5-ss))
