...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + identity + self.alpha * mask.mul(identity)
        out = self.relu(out)
...
class ResNet_last_attr(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer4 = self._make_layer(block, 1536, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(1536 * block.expansion + 256, 2048)
        self.bn_fc = nn.BatchNorm1d(num_features=2048)
        ...
        self.fc_e1_1 = nn.Linear(4096, 4096)
        self.fc_e1_1_1 = nn.Linear(4096, 4096)
        self.bn_e1 = nn.BatchNorm1d(num_features=4096)
        self.fc_e2 = nn.Linear(4096, num_classes)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            elif isinstance(m, (nn.BatchNorm2d, nn.GroupNorm)):
...
class Generator_paper1order1(nn.Module):
    ...
    def forward(self, z, y):
        ...
        img1, img2 = img.chunk(2, 0)
        # y = y.view(y.shape[0], 100, 1, 1)
        y_hat = self.conv_y(y)
        y_hat1, y_hat2 = y_hat.chunk(2, 0)
...
class Generator_paper1order2(nn.Module):
    def __init__(self):
        ...
        self.conv_blocks = nn.Sequential(
            ...
            # input batch_size*1024*4*4
            nn.ConvTranspose2d(in_channels=1024, out_channels=1024, kernel_size=4, stride=2, padding=1),  # up2
            # up2:4-2-1 up4:4-4-0 up4:8 4 2
            nn.BatchNorm2d(num_features=1024),
            nn.ReLU(inplace=True),
    ...
    def forward(self, z, y):
        ...
        out = z.view(z.shape[0], 100, 1, 1)
        img = self.conv_blocks(out)
        img1, img2 , img3= img.chunk(3, 0)
        # y = y.view(y.shape[0], 100, 1, 1)
        y_hat = self.conv_y(y)
        y_hat1, y_hat2, y_hat3 = y_hat.chunk(3, 0)
...
class Generator_paper1order3(nn.Module):
    def __init__(self):
        ...
        self.conv_blocks = nn.Sequential(
            ...
            # input batch_size*512*16*16//////512*8*8
            nn.ConvTranspose2d(in_channels=512, out_channels=512, kernel_size=8, stride=4, padding=2),  # 4
            # up2:4-2-1 up4:4-4-0 up4:8 4 2
            nn.BatchNorm2d(num_features=256),
            nn.ReLU(inplace=True),
            
            # input batch_size*256*64*64//////256*16*16
            nn.ConvTranspose2d(in_channels=512, out_channels=12, kernel_size=8, stride=4, padding=2),  # 4
            # up2:4-2-1 up4:4-4-0 up4:8 4 2
            # nn.LeakyReLU(inplace=True),
    ...
    def forward(self, z, y):
        ...
        out = z.view(z.shape[0], 100, 1, 1)
        img = self.conv_blocks(out)
        img1, img2, img3, img4 = img.chunk(4, 0)
        # y = y.view(y.shape[0], 100, 1, 1)
        y_hat = self.conv_y(y)
        y_hat1, y_hat2, y_hat3, y_hat4 = y_hat.chunk(4, 0)
