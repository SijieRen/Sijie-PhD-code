...
class Clas_ppa_train(data.Dataset):
    def __init__(self,
        ...
        workbook1 = xlrd.open_workbook(
                "../dataset/21-01-19-grade1and6-std-2-phase-1.xls")
        ...
        for rows in range(1, sheet1.nrows):
            if sheet1.row_values(rows)[1] == fold:
                ...
                self.target_RA_A.append(sheet1.row_values(rows)[2])  ###target RA_A-13 /// RA_P-16
                if filename == 'disease':
                    self.feature_all_1.append(
                    np.array(sheet1.row_values(rows))[3:24].astype('float32'))
                else:
                    self.feature_all_1.append(
                    np.array(sheet1.row_values(rows))[3:22].astype('float32'))
...
def train_baseline(phase,
    ...
    for batch_idx, (data_1, target_RA, feature_1,) in enumerate(train_loader):
        ...
        image_sequence += target_RA.size(0)
        print('pa pa pa !')
...
def save_results_baseline(args,
    ...
    args.logger.info(
        'In this epoch : {}, for margin {},test acc is {}'.format(args.best_val_acc_epoch, args.margin,
                                                                  full_results[args.best_val_acc_epoch]['test_results'][
                                                                      'acc_1']))
    args.logger.info(
...
def main():
    ...
    model_2 = MLP(in_channels=args.in_ch, out_channel=args.class_num).cuda()
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            train_results = train_baseline('train', args, model_2, train_loader,
                                           optimizer_M_2, epoch)
            test_results = test_baseline('test', args, model_2, test_loader,
                                         optimizer_M_2, epoch)
            val_results = test_baseline('val', args, model_2, val_loader,
                                        optimizer_M_2, epoch)
