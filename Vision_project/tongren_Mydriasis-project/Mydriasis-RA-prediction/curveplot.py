...
print(len(data))
print(data[1]['train_results']['loss'])
for i in range(1, len(data)+1):
    #lossM.append(data[i]['train_results']['loss_M'])
    #lossM1.append(data[i]['train_results']['loss_M1'])
    loss_train.append(data[i]['train_results']['loss'])
    loss_test.append(data[i]['test_results']['loss'])
    loss_val.append(data[i]['val_results']['loss'])
    
    acc_train.append(data[i]['train_results']['acc_2'])
    acc_test.append(data[i]['test_results']['acc_2'])
    acc_val.append(data[i]['val_results']['acc_2'])
...
plt.plot(ep[:],loss_train[:],  c='green',label = 'loss_trian')
plt.plot(ep[:],loss_test[:],  c='red',label = 'loss_test')
plt.plot(ep[:],loss_val[:],  c='blue',label = 'loss_val')
print(len(ep))
print(len(acc_train))
plt.plot(ep[:],acc_train[:],  c='yellow',label = 'acc_trian')
plt.plot(ep[:],acc_test[:],  c='black',label = 'acc_test')
plt.plot(ep[:],acc_val[:],  c='maroon',label = 'acc_val')
