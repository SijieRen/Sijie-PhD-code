...
def get_opts():
    ...
    parser.add_argument('--feature_list', type=int, nargs='+', default=[6,16,21,22,26,27,28,31,36,37], help='')
    ...
    parser.add_argument('--double_pred', type=int, default=0)
    ...
    parser.add_argument('--modality', type=str, default='normal')
    parser.add_argument('--in_ch', type=int, default=20)
    ...
    save_dir = '../results/Mydriasis_%s/%s_%s_%f_%f_size_%s_ep_%d_%d_%s_%s/' % (
        args.sequence,
        time_stamp.strftime(
            '%Y-%m-%d-%H-%M-%S'),
        args.optimizer,
...
def adjust_learning_rate(optimizer, epoch, args, lr):
    ...
    lr_ = lr * (args.lr_decay ** (epoch // args.lr_controler))
